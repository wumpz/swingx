...
public class Rectangle2DPropertyEditor extends PropertyEditorSupport {
	...
	public String getJavaInitializationString() {
		...
		return rect == null
				? "null"
				: "new java.awt.geom.Rectangle2D.Double(" + rect.getX() + ", " + rect.getY() + ", " + rect.getWidth()
						+ ", " + rect.getHeight() + ")";
	}
	...
	public void setAsText(String text) throws IllegalArgumentException {
		...
		try {
			Rectangle2D val = (Rectangle2D)
					PropertyEditorUtil.createValueFromString(text, 4, Rectangle2D.Double.class, double.class);
			setValue(val);
		} catch (Exception e) {
			throw new IllegalArgumentException(
					"The input value " + originalParam + " is not formatted correctly. Please "
							+ "try something of the form [x,y,w,h] or [x , y , w , h] or [x y w h]",
					e);
		}
	}
