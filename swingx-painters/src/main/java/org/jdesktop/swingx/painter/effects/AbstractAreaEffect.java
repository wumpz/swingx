...
public class AbstractAreaEffect implements AreaEffect {
	...
	public void apply(Graphics2D g, Shape clipShape, int width, int height) {
		...
		width = (int)
				(clipShape.getBounds2D().getWidth() + clipShape.getBounds2D().getX());
		height = (int)
				(clipShape.getBounds2D().getHeight() + clipShape.getBounds2D().getY());
		Rectangle effectBounds =
				new Rectangle(0, 0, width + getEffectWidth() * 2 + 1, height + getEffectWidth() * 2 + 1);
		...
		if (isShapeMasked()) {
			...
			g.drawImage(
					clipImage,
					-getEffectWidth() + (int) getOffset().getX(),
	...
	protected void paintBorderGlow(Graphics2D g2, Shape clipShape, int width, int height) {
		...
		if (isShouldFillShape()) {
			...
			if (inside) {
				...
				Area a1 = new Area(
						new Rectangle((int) -offset.getX() - 20, (int) -offset.getY() - 20, width + 40, height + 40));
	...
	public void setRenderInsideShape(boolean renderInsideShape) {
		...
		propertyChangeSupport.firePropertyChange(
				"renderInsideShape", new Boolean(oldRenderInsideShape), new Boolean(renderInsideShape));
	}
	...
	public void setShouldFillShape(boolean shouldFillShape) {
		...
		propertyChangeSupport.firePropertyChange(
				"shouldFillShape", new Boolean(oldShouldFillShape), new Boolean(shouldFillShape));
	}
