...
public class DefaultDateSelectionModelTest extends AbstractTestDateSelectionModel {
	...
	private static final Logger LOG = Logger.getLogger(DefaultDateSelectionModelTest.class.getName());
	...
	public void testLowerBoundJustBelow() {
		...
		model.setSelectionInterval(calendar.getTime(), calendar.getTime());
		assertEquals(null, model.getFirstSelectionDate(), "future selection just above must be rejected");
	}
	/**
	 *  respect upper bound - the bound itself
	 *  a valid selection.
	...
	public void testLowerBoundJustAbove() {
		...
		calendar.add(Calendar.HOUR_OF_DAY, +1);
		model.setSelectionInterval(calendar.getTime(), calendar.getTime());
		assertEquals(calendar.getTime(), model.getFirstSelectionDate(), "selected just below upper bound");
	}
