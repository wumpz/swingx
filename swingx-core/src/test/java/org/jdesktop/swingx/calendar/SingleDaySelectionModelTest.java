...
public class SingleDaySelectionModelTest extends AbstractTestDateSelectionModel {
	...
	private static final Logger LOG = Logger.getLogger(SingleDaySelectionModelTest.class.getName());
	...
	public void testIsSelectedAllInSameDay() {
		...
		assertTrue(
				...
				"every date contained in today must be regarded as selected - startofday");
		assertTrue(
				model.isSelected(endOfDay(today)),
				"every date contained in today must be regarded as selected - endofday");
	}
	...
	public void testSelectionSetNotFireIfSameSingle() {
		...
		model.setSelectionInterval(date, date);
		assertEquals(0, report.getEventCount(), "selection must not fire on selecting already selected date");
	}
	...
	public void testDateSelectionClearSelectionNotFireIfUnselected() {
		...
		assertEquals(0, report.getEventCount(), "selection must not fire on clearing empty selection");
	}
	...
	public void testDateSelectionClearSelectionFireIfSelected() {
		...
		DateSelectionReport report = new DateSelectionReport(model);
		model.clearSelection();
		assertEquals(1, report.getEventCount(), "selection must fire on clearing selection");
		assertEquals(
				DateSelectionEvent.EventType.SELECTION_CLEARED,
				report.getLastEventType(),
				"event type must be SELECTION_CLEARED");
	}
	...
	public void testDateSelectionSetSelectionFire() {
		...
		assertEquals(1, report.getEventCount(), "selection must fire on selection");
		assertEquals(DateSelectionEvent.EventType.DATES_SET, report.getLastEventType(), "event type must be DATES_SET");
	}
	...
	public void testLowerBoundSameDay() {
		...
		model.setSelectionInterval(today, today);
		assertTrue(model.isSelected(today), "same day as lower bound is allowed");
	}
	...
	public void testUpperBoundSameDay() {
		...
		assertTrue(model.isSelected(today), "same day as upper bound is allowed");
	}
	...
	public void testUnselectableDatesCompleteDay() {
		...
		assertTrue(model.isUnselectableDate(today), "raw today must be unselectable");
		assertTrue(model.isUnselectableDate(startOfDay(today)), "start of today must be unselectable");
		assertTrue(model.isUnselectableDate(endOfDay(today)), "end of today must be unselectable");
	...
	public void testRemoveInterval() {
		...
		assertTrue(
				...
				"selection must be empty: selected today and removed interval [yesterday, tomorrow]");
	}
	...
	public void testUnselectableDatesSetClearsSelection() {
		...
		assertTrue(model.isSelectionEmpty(), "today must be removed");
	}
