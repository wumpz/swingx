...
public class JXTreeUnitTest extends InteractiveTestCase {
	...
	public void testNextMatch() {
		...
		tree.expandAll();
		assertEquals(
				null, tree.getNextMatch("b", 0, Bias.Forward), "must not find a match for 'b', all start with 'r'");
	}
	...
	public void testDefaultRendererOnInit() {
		...
		assertSame(
				tree.getCreatedDefaultRenderer(),
				tree.getWrappedCellRenderer(),
				"sanity: default renderer used as wrappee");
	}
	...
	public void testDelegatingRendererUseDefault() {
		JXTree list = new JXTree();
		TreeCellRenderer defaultRenderer = list.createDefaultCellRenderer();
		assertEquals(DefaultXTreeCellRenderer.class, defaultRenderer.getClass(), "sanity: creates default");
		DelegatingRenderer renderer = (DelegatingRenderer) list.getCellRenderer();
	...
	public void testIconSetOnRendererTrueOverwrite() {
		...
		tree.setCellRenderer(renderer);
		assertEquals(null, renderer.getLeafIcon(), "renderer leaf icon must be overwritten by tree's leaf icon");
	}
	...
	public void testEditorListenerOnXTree() {
		...
		JXTree tree = createEditingTree();
		DefaultTreeCellEditor cellEditor = (DefaultTreeCellEditor) tree.getCellEditor();
		assertEquals(coreCount + 1, cellEditor.getCellEditorListeners().length, "need one more listener than core");
	}
	...
	private void assertXTreeInit(JXTree tree) {
		...
		assertTrue(tree.getCellEditor() instanceof DefaultXTreeCellEditor, "Editor must be DefaultXTreeCellEditor");
