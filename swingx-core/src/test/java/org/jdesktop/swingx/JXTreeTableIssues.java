...
public class JXTreeTableIssues extends InteractiveTestCase {
	...
	public void interactivePrototype() {
		ColumnFactory factory = new ColumnFactory() {
			...
			protected boolean isHierarchicalPrototype(JXTable table, TableColumnExt columnExt) {
			...
			protected int calcHierarchicalPrototypeWidth(JXTreeTable table, TableColumnExt columnExt) {
				...
				Component comp = dummy.getTreeCellRendererComponent(
						renderer, columnExt.getPrototypeValue(), false, false, false, -1, false);
		...
		table.getColumnExt(0)
				.setPrototypeValue(
						"long longer longest still not enough to really see some effect of the prototype if availabel");
	...
	public void interactiveScrollAlternateHighlightDefaultModel() {
		...
		final Highlighter hl =
				HighlighterFactory.createAlternateStriping(UIManager.getColor("Panel.background"), Color.WHITE);
	...
	public void testTableEventUpdateOnTreeTableSetValueForRoot()
		...
		SwingUtilities.invokeAndWait(new Runnable() {
			...
			public void run() {
				...
				assertEquals(
						1,
						report.getUpdateEventCount(),
						"the event type must be update " + TableModelReport.printEvent(report.getLastEvent()));
	...
	public void interactiveTreeTableModelAdapterDeleteUpdate() {
		final DefaultTreeTableModel customTreeTableModel =
				(DefaultTreeTableModel) createCustomTreeTableModelFromDefault();
		...
		final JXFrame frame = wrapWithScrollingInFrame(
				table, xtree, "JXTreeTable.TreeTableModelAdapter: Inconsistency firing update on recursive delete");
		final MutableTreeTableNode deletedNode =
				(MutableTreeTableNode) table.getPathForRow(6).getLastPathComponent();
		MutableTreeTableNode child1 =
				(MutableTreeTableNode) table.getPathForRow(6 + 1).getLastPathComponent();
		MutableTreeTableNode child2 =
				(MutableTreeTableNode) table.getPathForRow(6 + 2).getLastPathComponent();
		MutableTreeTableNode child3 =
				(MutableTreeTableNode) table.getPathForRow(6 + 3).getLastPathComponent();
		MutableTreeTableNode child4 =
				(MutableTreeTableNode) table.getPathForRow(6 + 4).getLastPathComponent();
		...
		final ActionListener l = new ActionListener() {
			...
			private void removeChild(
					final DefaultTreeTableModel customTreeTableModel,
					final MutableTreeTableNode deletedNode,
					final MutableTreeTableNode[] children,
					final String[] values) {
	...
	public void interactiveTreeTableModelAdapterUpdate() {
		...
		xtree.expandAll();
		final JXFrame frame = wrapWithScrollingInFrame(
				table, xtree, "JXTreeTable.TreeTableModelAdapter: Inconsistency firing update");
		Action changeValue = new AbstractAction("change sports to games") {
			@Override
			public void actionPerformed(ActionEvent e) {
				String newValue = "games";
				table.getTreeTableModel()
						.setValueAt(newValue, table.getPathForRow(6).getLastPathComponent(), 0);
			}
		};
		addAction(frame, changeValue);
	...
	public void interactiveTreeTableModelAdapterDelete() {
		...
		Action changeValue = new AbstractAction("delete first child of sports") {
			@Override
			public void actionPerformed(ActionEvent e) {
				MutableTreeTableNode firstChild =
						(MutableTreeTableNode) table.getPathForRow(6 + 1).getLastPathComponent();
	...
	public void interactiveTreeTableModelAdapterMutateSelected() {
		...
		xtree.expandAll();
		final JXFrame frame = wrapWithScrollingInFrame(
				table, xtree, "JXTreeTable.TreeTableModelAdapter: Inconsistency firing delete expanded folder");
		Action changeValue = new AbstractAction("delete selected node") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (row < 0) return;
				MutableTreeTableNode firstChild =
						(MutableTreeTableNode) table.getPathForRow(row).getLastPathComponent();
				((DefaultTreeTableModel) customTreeTableModel).removeNodeFromParent(firstChild);
			}
		};
		addAction(frame, changeValue);
		Action changeValue1 = new AbstractAction("insert as first child of selected node") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				((DefaultTreeTableModel) customTreeTableModel).insertNodeInto(newChild, firstChild, 0);
			}
		};
	...
	public void interactiveTreeTableLinkRendererSimpleText() {
		...
		tree.setTreeCellRenderer(
				new DefaultTreeRenderer( // provider));
	...
	public void interactiveTextAreaTreeTable() {
		...
		TreeCellRenderer renderer = new DefaultTreeRenderer(new WrappingProvider(new TextAreaProvider()));
	...
	private TreeTableModel createTreeTableModelWithLongNode() {
		MutableTreeTableNode root =
				createLongNode("some really, maybe really really long text -  " + "wrappit .... where needed ");
		root.insert(
				createLongNode("another really, maybe really really long text -  "
	...
	public void interactiveTreeTableClipIssueWrappingProvider() {
		...
		JXFrame frame = wrapWithScrollingInFrame(treeTable, tree, "treetable and tree with wrapping provider");
	...
	public void interactiveTreeTableClipIssueDefaultRenderer() {
		...
		tree.collapseRow(0);
		JXFrame frame = wrapWithScrollingInFrame(treeTable, tree, "JXTreeTable vs. core JTree: default renderer");
		Action revalidate = new AbstractActionExt("revalidate") {
	...
	public void interactiveTreeTableClipIssueCustomDefaultRenderer() {
		...
		JXFrame frame = wrapWithScrollingInFrame(
				treeTable, tree, "JXTreeTable vs. JTree: custom default renderer without clip");
		Action revalidate = new AbstractActionExt("revalidate") {
	...
	public void testLeadSelectionFromTable() {
		...
		assertEquals(
				treeTable.getSelectionModel().getLeadSelectionIndex(),
	...
	public void testLeadAfterRemoveSelectionFromTree() {
		...
		treeTable
				...
				.removeSelectionPath(treeTable.getTreeSelectionModel().getLeadSelectionPath());
		assertEquals(
				treeTable.getSelectionModel().getLeadSelectionIndex(),
				treeTable.getTreeSelectionModel().getLeadSelectionRow());
	}
