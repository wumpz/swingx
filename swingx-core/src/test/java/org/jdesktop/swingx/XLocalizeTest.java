...
public class XLocalizeTest extends InteractiveTestCase {
	...
	public void testLocaleDialogPropertyNotificationInListener() {
		...
		PropertyChangeListener report = new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				...
				//                assertTrue("locale property changed, instead: " + evt.getPropertyName(),
				// "locale".equals(evt.getPropertyName()));
				...
				assertEquals(altUIValue, altName, "name must be updated before fire propertyChange");
			}
		};
	...
	public void testLocaleFindBarPropertyNotificationInListener() {
		...
		String uiValue = UIManagerExt.getString(prefix + actionCommand, findPanel.getLocale());
		// sanity
		assertNotNull(uiValue);
		assertEquals(name, uiValue);
	...
	public void testLocaleTablePropertyNotification() {
		...
		String name = (String) action.getValue(Action.NAME);
		String uiValue = UIManagerExt.getString(prefix + actionCommand, table.getLocale());
		// sanity
		assertNotNull(uiValue);
		assertEquals(name, uiValue);
	...
	private void assertLocaleActionUpdate(JComponent table, String prefix, String actionCommand) {
		...
		table.setLocale(alternative);
		String altUIValue = UIManagerExt.getString(prefix + actionCommand, table.getLocale());
		// sanity
		assertNotNull(altUIValue);
		// sanity to track unexpected failure during refactoring
		assertFalse(uiValue.equals(altUIValue), "new uiValue  must be different: " + uiValue + "/" + altUIValue);
		String altName = (String) action.getValue(Action.NAME);
		// here are the real asserts
		assertFalse(name.equals(altName), "new action name must be different: " + name + "/" + altName);
	...
	public void testGetLocaleUIDefaults() {
		...
		assertFalse(
				defaultValue.equals(alternativeValue),
				"values must be different: " + defaultValue + "/" + alternativeValue);
	}
