...
public class BasicMonthViewUITest extends InteractiveTestCase {
	...
	public void testLocaleByProviderMonthRendering() {
		...
		JComponent component = handler.prepareRenderingComponent(monthView, calendar, CalendarState.TITLE);
	 ...
	 */
	private void assertWeekNumbers(
			Locale locale, int year, int month, int expectedDay, int expectedWeek, int expectedWeekNumber) {
	...
	public void testZoomableKeepsMonthStringBackground() {
		...
		assertEquals(
				monthView.getMonthStringBackground(),
				((BasicMonthViewUI) monthView.getUI())
	...
	public void testZoomableCustomHeader() {
		UIManager.put(
				CalendarHeaderHandler.uiControllerID, "org.jdesktop.swingx.plaf.basic.SpinningCalendarHeaderHandler");
		JXMonthView monthView = new JXMonthView();
		monthView.setZoomable(true);
		...
		try {
			assertTrue(
					header instanceof SpinningCalendarHeaderHandler.SpinningCalendarHeader,
					"expected SpinningCalendarHeader but was " + header.getClass());
		} finally {
	...
	public void testZoomableAddsCalendarHeader() {
		...
		assertTrue(
				...
				"expected BasicCalendarHeader but was " + header.getClass());
	}
	...
	public void testDayInMonthForDayOfWeekHeader() {
		...
		assertEquals(
				null,
				ui.getDayInMonth(month, BasicMonthViewUI.DAY_HEADER_ROW, BasicMonthViewUI.LAST_DAY_COLUMN),
				"header grid position must return null date");
	}
	...
	public void testDayInMonthForWeekNumberHeader() {
		...
		assertEquals(
				null,
				ui.getDayInMonth(month, BasicMonthViewUI.FIRST_WEEK_ROW + 3, BasicMonthViewUI.WEEK_HEADER_COLUMN),
				"header grid position must return null date");
	}
	 ...
	 */
	private void assertDateToDayGrid(BasicMonthViewUI ui, Date month, int dayRow, int dayColumn) {
	...
	public void testDateToMonthBoundsNotVisible() {
		...
		assertEquals(null, ui.getMonthBounds(uiCalendar.getTime()), "leading dates must return null bounds");
	}
	...
	public void testDateToDayBoundsLeadingDate() {
		...
		CalendarUtils.startOfWeek(uiCalendar);
		assertFalse(month == uiCalendar.get(Calendar.MONTH), "sanity - we have leading dates in the month");
		assertEquals(null, ui.getDayBounds(uiCalendar.getTime()), "leading dates must return null bounds");
	}
	...
	public void testDaySize() {
		...
		Rectangle monthBounds = ui.getMonthBoundsAtLocation(20, 20);
		Rectangle dayBounds =
				ui.getDayBoundsAtLocation(monthBounds.x + 2, monthBounds.y + ui.getMonthHeaderHeight() + 2);
	...
	public void testMonthBoundsFromLogicalRToL() {
		...
		Rectangle monthBounds11 = ui.getMonthBoundsAtLocation(20, monthBounds.y + 20 + monthBounds.height);
	...
	public void testMonthBoundsFromLogicalLToR() {
		...
		Rectangle monthBounds11 = ui.getMonthBoundsAtLocation(
				monthBounds.x + 20 + monthBounds.width, monthBounds.y + 20 + monthBounds.height);
		// second row, second column in logical coordinates
		assertEquals(monthBounds11, ui.getMonthBounds(1, 1));
	}
	...
	public void testDayAtLocationDayHeaderNull() {
		...
		Rectangle monthBounds = ui.getMonthBoundsAtLocation(20, 20);
		// same for LToR
		Date date = ui.getDayAtLocation(monthBounds.x + 2, monthBounds.y + ui.getMonthHeaderHeight() + 2);
	...
	public void testDayBounds() {
		...
		Rectangle dayBoundsRToL =
				uiRToL.getDayBoundsAtLocation(monthBounds.x + 2, monthBounds.y + uiRToL.getMonthHeaderHeight());
		// same for LToR
		Rectangle dayBoundsLToR =
				uiLToR.getDayBoundsAtLocation(monthBounds.x + 2, monthBounds.y + uiLToR.getMonthHeaderHeight());
		assertEquals(dayBoundsLToR, dayBoundsRToL, "day bounds must be independent of orientation");
	...
	public void testDayGridPositionAtLocationRows() {
		...
		for (int row = 0; row <= BasicMonthViewUI.WEEKS_IN_MONTH; row++) {
			...
			assertEquals(calendarRow, dayGridPosition.y, "calendarRow at absolute row " + row);
			calendarRow++;
		}
	}
	...
	public void testDayGridPositionAtLocationColumnsLToR() {
		...
		for (int column = 0; column < BasicMonthViewUI.DAYS_IN_WEEK; column++) {
			...
			// first row below month header == days of week header
			Point dayGridPosition = ui.getDayGridPositionAtLocation(location.x, location.y);
			assertEquals(calendarColumn, dayGridPosition.x, "calendarColumn at absolute column " + column);
			calendarColumn++;
		}
	}
	...
	public void testDayGridPositionAtLocationDayHeader() {
		...
		Point dayGridPosition = ui.getDayGridPositionAtLocation(location.x, location.y);
		assertEquals(
				BasicMonthViewUI.DAY_HEADER_ROW, dayGridPosition.y, "first row below header must be day column header");
	}
	...
	public void testDayGridPositionAtLocationLastColumnRToL() {
		...
		Point dayGridPosition = ui.getDayGridPositionAtLocation(location.x, location.y);
		assertEquals(BasicMonthViewUI.LAST_DAY_COLUMN, dayGridPosition.x, "last logical column in RToL");
	}
	...
	public void testDayGridPositionAtLocationFirstColumnRToL() {
		...
		Point dayGridPosition = ui.getDayGridPositionAtLocation(location.x, location.y);
		assertEquals(BasicMonthViewUI.FIRST_DAY_COLUMN, dayGridPosition.x, "last logical column in RToL");
	}
	...
	public void testDayGridPositionAtLocationWeekHeaderRToL() {
		...
		Point dayGridPosition = ui.getDayGridPositionAtLocation(location.x, location.y);
		assertEquals(BasicMonthViewUI.WEEK_HEADER_COLUMN, dayGridPosition.x, "weeks of year column in RTL");
	}
	...
	public void testMonthFromGrid() {
		...
		assertEquals(monthField, first.get(Calendar.MONTH), "logical coordinates must be independent of orientation");
	}
	...
	public void testMonthBoundsAtLocation() {
		...
		// bounds of first
		assertEquals(monthBoundsLToR, monthBoundsRToL, "bounds of left-most month must be equal");
		Rectangle monthBoundsTwoRToL = uiRToL.getMonthBoundsAtLocation(monthBoundsRToL.width + 20, 20);
		Rectangle monthBoundsTwoLToR = uiRToL.getMonthBoundsAtLocation(monthBoundsLToR.width + 20, 20);
		assertEquals(monthBoundsTwoLToR, monthBoundsTwoRToL, "bounds of right-most month must be equal");
	}
	...
	private Point getLocationInColumn(BasicMonthViewUI ui, int columnOfDayBoxes) {
		...
		Dimension dayBounds = ui.getDaySize();
		Point location = new Point(
				monthBounds.x + columnOfDayBoxes * dayBounds.width + 2, monthBounds.y + ui.getMonthHeaderHeight() + 2);
		return location;
	}
	...
	private Point getLocationInRow(BasicMonthViewUI ui, int rowOfDayBoxes) {
		...
		Dimension dayBounds = ui.getDaySize();
		Point location = new Point(
				monthBounds.x + 2, monthBounds.y + ui.getMonthHeaderHeight() + rowOfDayBoxes * dayBounds.height + 2);
		return location;
	}
	 ...
	 * @return
	 */
	private BasicMonthViewUI getRealizedMonthViewUI(ComponentOrientation co, boolean isShowingWeekNumbers) {
		JXMonthView monthView = new JXMonthView();
	...
	private void assertWeekdays(JXMonthView monthView, Locale french) {
		...
		String[] weekdays = DateFormatSymbols.getInstance(french).getShortWeekdays();
