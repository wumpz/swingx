...
public class ListSortUITest extends InteractiveTestCase {
	...
	public void testSelectionAfterSort() {
		...
		assertEquals(ascendingListModel.getSize() - (testRow + 1), index, "last row must be selected after sorting");
	}
	...
	public void testSelectionAfterAddAbove() {
		...
		assertEquals(2, list.getSelectedIndex(), "selected must have moved after adding at start");
	}
	...
	public void testSelectionAfterDeleteAbove() {
		...
		// remove first
		ascendingListModel.remove(0);
		assertEquals(0, list.getSelectedIndex(), "first row must be selected removing old first");
	}
