...
public class RawNimbusIssues extends InteractiveTestCase {
	...
	public void testColorInCompleteFireChange() throws Exception {
		...
		try {
			...
			if (colors.get(0).getRGB() != rgbOrg) {
				assertTrue(report.hasEvents(), "lookandFeelDefaults must have fired color change notification");
			}
		} finally {
	...
	public void testColorCompleteInstalled() throws Exception {
		...
		try {
			...
			assertSame(
					colors.get(0),
					UIManager.getColor(ALTERNATE_ROW_COLOR),
					"Color installed when firing property change");
			if (!report.hasEvents()) {
				int rgbOrg = rgb.get(0);
				assertEquals(rgbOrg, colors.get(0).getRGB(), "Color must be unchanged compared to original");
			}
		} finally {
			removeListeners(l, report);
		}
	...
	private void removeListeners(
			PropertyChangeListener managerListener, PropertyChangeListener lookAndFeelDefaultsListener) {
