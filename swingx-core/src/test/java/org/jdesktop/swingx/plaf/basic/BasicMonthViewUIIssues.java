...
public class BasicMonthViewUIIssues extends InteractiveTestCase {
	...
	private static final Logger LOG = Logger.getLogger(BasicMonthViewUIIssues.class.getName());
	...
	public void testTimeZoneCairoCalendarUtils() {
		...
		for (int week = 0; week < 6; week++) {
			for (int day = 0; day < 7; day++) {
				...
				assertTrue(CalendarUtils.isStartOfDay(calendar), "must be start of day " + calendar.getTime());
			}
			assertTrue(CalendarUtils.isStartOfWeek(calendar), "must be start of week " + calendar.getTime());
		}
	}
	...
	public void testDayBoundsLeadingDatesNull() {
		...
		assertEquals(null, ui.getDayBounds(calendar.getTime()), "leading dates must return null bounds");
	}
