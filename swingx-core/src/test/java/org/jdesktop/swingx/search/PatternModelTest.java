...
public class PatternModelTest {
	...
	static final String middleStartAnchor = "some" + startAnchor + "one";
	static final String middleEndAnchor = "some" + endAnchor + "one";
	...
	public void testFoundIndexAutoAdjustBackward() {
		...
		assertTrue(propertyReport.hasEvents("foundIndex"), "changing foundIndex must have fired foundIndex property");
	}
	...
	public void testCaseSensitive() {
		...
		assertTrue(model.getPattern().matcher("JTABLE").find(), "must find not case sensitive by default");
		model.addPropertyChangeListener(propertyReport);
		model.setCaseSensitive(true);
		assertTrue(
				propertyReport.hasEvents("caseSensitive"), "changing case sensitive must fire casesensitive property");
		assertTrue(propertyReport.hasEvents("pattern"), "changing case sensitive must fire pattern property");
	}
	...
	public void testMatchRule() {
		...
		assertEquals(PatternModel.REGEX_MATCH_RULES, model.getRegexCreatorKey(), "search string mode must be");
		// default searchCategory
		assertEquals(PatternModel.MATCH_RULE_CONTAINS, model.getMatchRule(), "search category must be ");
	...
	public void testRawMiddleAnchorTokens() {
		...
		assertTrue(
				patternModel.getPattern().matcher(literalAnchor).find(),
				"must find literal containing startAnchor " + literalAnchor);
