...
public class JXTreeIssues extends JXTreeUnitTest {
	...
	public void interactiveUpdateUICoreTreeDefaultRenderer() {
		...
		JXFrame frame =
				wrapWithScrollingInFrame(tree, treeSetRenderer, "JTree/core renderer: updateUI must update renderer");
	...
	public void interactiveUpdateUIXTreeDefaultRenderer() {
		...
		treeSetRenderer.setEditable(true);
		JXFrame frame =
				wrapWithScrollingInFrame(tree, treeSetRenderer, "JXTree/core renderer: updateUI must update renderer");
		addComponentOrientationToggle(frame);
		show(frame);
	}
	...
	public void interactiveUpdateUIXTreeXRenderer() {
		...
		treeSetRenderer.setEditable(true);
		JXFrame frame =
				wrapWithScrollingInFrame(treeSetRenderer, "JXTree/x renderer: updateUI must update renderer/editor");
		addComponentOrientationToggle(frame);
		show(frame);
	}
	...
	public void interactiveProviderStringValue() {
		...
		tree.setRowHeight(22);
		JXFrame frame = wrapWithScrollingInFrame(tree, "Custom StringValue on WrappingProvider, default icons");
		//        addAction(frame, toggleClosedIcon);
		frame.setVisible(true);
	}
