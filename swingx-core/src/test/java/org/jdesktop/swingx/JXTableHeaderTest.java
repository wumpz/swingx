...
public class JXTableHeaderTest extends InteractiveTestCase {
	private static final Logger LOG = Logger.getLogger(JXTableHeaderTest.class.getName());
	...
	public void testPreferredHeight() {
		...
		assertEquals(
				prefRendererSize.height,
				tableHeader.getPreferredSize().height,
				"Header pref height must respect renderer");
	}
	...
	public void testColumnToolTip() {
		...
		TableColumnExt columnExt = table.getColumnExt(0);
		JXTableHeader tableHeader = (JXTableHeader) table.getTableHeader();
		MouseEvent event = new MouseEvent(tableHeader, 0, 0, 0, 40, 5, 0, false);
	...
	public void testHeaderVisibleWithoutColumns() {
		...
		assertEquals(table.getWidth(), table.getTableHeader().getWidth(), "headerWidth must be table width");
	}
	...
	public void interactiveHeaderVisible() {
		...
		Action action = new AbstractAction("toggle model") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				table.setModel(columnCount > 0 ? new DefaultTableModel() : new DefaultTableModel(10, 2));
			}
		};
