...
public class JXDatePickerTest extends InteractiveTestCase {
	...
	public void testEnabledKeptOnUpdateUI() {
		...
		assertEquals(picker.isEnabled(), picker.getEditor().isEnabled(), "editor enabled must follow picker enabled");
		picker.updateUI();
		assertFalse(picker.isEnabled(), "sanity: picker disabled");
		for (int i = 0; i < picker.getComponentCount(); i++) {
			assertEquals(
					picker.isEnabled(), picker.getComponent(i).isEnabled(), "child enabled must follow picker enabled");
		}
	}
	...
	public void testEditableKeptOnUpdateUI() {
		...
		assertEquals(picker.isEditable(), picker.getEditor().isEditable(), "editor enabled must follow picker enabled");
		picker.updateUI();
		assertFalse(picker.isEditable(), "sanity: picker disabled");
		assertEquals(picker.isEditable(), picker.getEditor().isEditable(), "editor enabled must follow picker enabled");
	}
	...
	public void testPickerFormatsNotNull() {
		...
		picker.getEditor()
				.setFormatterFactory(new DefaultFormatterFactory(new DatePickerFormatter((DateFormat[]) null)));
		assertNotNull(picker.getFormats(), "picker format array must not be null");
	}
	...
	public void testLinkPanelRemovedAdded() {
		...
		assertLinkPanelBindings(linkPanel, true);
		assertSame(oldParent, linkPanel.getParent(), "linkPanel must be added to same parent");
		frame.dispose();
	}
	...
	public void assertKeyBindings(JComponent comp, Object actionKey, boolean bound) {
		boolean hasAncestorBinding =
				hasBinding(comp.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT), actionKey);
		boolean hasFocusedBinding = hasBinding(comp.getInputMap(JComponent.WHEN_FOCUSED), actionKey);
		boolean hasInFocusedBinding = hasBinding(comp.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW), actionKey);
		boolean hasBinding = hasAncestorBinding || hasFocusedBinding || hasInFocusedBinding;
		assertEquals(bound, hasBinding, "component has keybinding for " + actionKey);
	}
	...
	public void testTogglePopupAction() {
		...
		Object actionKey = picker.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
				.get(space);
	...
	public void testCommitCancelFromMonthViewCancel() {
		...
		assertEquals(1, report.getEventCount(), "must have receive 1 event after monthView cancel");
	...
	public void testCommitCancelFromMonthViewCommit() {
		...
		assertEquals(1, report.getEventCount(), "must have receive 1 event after monthView commit");
	 ...
	 */
	private void assertTimeZoneLinkFormat(JXDatePicker picker, TimeZone alternative) {
	 ...
	 * @param alternative the expected timeZone of the picker (for sanity only)
	 */
	private void assertTimeZoneDateFormats(JXDatePicker picker, TimeZone alternative) {
	...
	public void testSpuriousEnabledListening() {
		...
		assertFalse(button.isEnabled(), "Do not change the state of the sender during notification processing");
	}
	...
	public void testEditorListeners() {
		...
		assertEquals(defaultListenerCount + 1, picker.getEditor().getPropertyChangeListeners().length);
		picker.getUI().uninstallUI(picker);
		assertEquals(
	...
	public void testPrefSizeEmptyInsertText() {
		...
		picker.getEditor().setText("1");
		assertEquals(empty.width, picker.getPreferredSize().width, "pref width must be same while editing");
	}
	...
	public void testPrefSizeValueInsertText() {
		...
		// simulate editing (uncommitted)
		picker.getEditor().setText("1");
		assertEquals(withValue.width, picker.getPreferredSize().width, "pref width must be same while editing");
	}
	...
	public void testPrefSizeEmptyEditor() {
		...
		// null value
		picker.setDate(null);
		assertEquals(withValue.width, picker.getPreferredSize().width, "pref width must be same null value");
	}
