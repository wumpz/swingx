...
public class HighlighterTest extends InteractiveTestCase {
	...
	public void testBorderPaddingNull() {
		...
		assertEquals(border, allColored.getBorder(), "borderHighlighter without padding must not change the component");
	}
	...
	public void testBorderPaddingCompoundNotInner() {
		...
		assertEquals(
				border,
				((CompoundBorder) compound).getOutsideBorder(),
				"borderHighlighter with padding and outer compound must have outside");
		assertEquals(
				padding,
				((CompoundBorder) compound).getInsideBorder(),
				"borderHighlighter with padding and outer compound must have inside");
	}
	...
	public void testBorderPaddingReplace() {
		...
		empty.highlight(allColored, createComponentAdapter(allColored));
		assertEquals(padding, allColored.getBorder(), "borderHighlighter padding and not-compound must replace");
	}
	...
	public void testBorderPaddingSetBorderIfComponentBorderNull() {
		...
		empty.highlight(allColored, createComponentAdapter(allColored));
		assertEquals(padding, allColored.getBorder(), "borderHighlighter padding and null component border must set");
	}
	...
	private void assertBorderHLState(
			BorderHighlighter hl, HighlightPredicate predicate, Border border, boolean compound, boolean inner) {
		assertEquals(predicate, hl.getHighlightPredicate());
	...
	public void testConstructors() {
		...
		ColorHighlighter normalNever = new ColorHighlighter(HighlightPredicate.NEVER, background, foreground);
		assertColorsAndPredicate(normalNever, HighlightPredicate.NEVER, background, foreground, null, null);
		ColorHighlighter full = new ColorHighlighter(background, foreground, selectedBackground, selectedForeground);
		assertColorsAndPredicate(
				full, HighlightPredicate.ALWAYS, background, foreground, selectedBackground, selectedForeground);
		ColorHighlighter fullNever = new ColorHighlighter(
				HighlightPredicate.NEVER, background, foreground, selectedBackground, selectedForeground);
		assertColorsAndPredicate(
				fullNever, HighlightPredicate.NEVER, background, foreground, selectedBackground, selectedForeground);
	}

	private void assertColorsAndPredicate(
	...
	public void testColorHighlighterNoChangeNotification() {
		ColorHighlighter highlighter =
				new ColorHighlighter(HighlightPredicate.NEVER, Color.RED, Color.RED, Color.RED, Color.RED);
	...
	public void testHighlightPredicate() {
		ColorHighlighter highlighter = new ColorHighlighter(unselectedBackground, unselectedForeground);
		highlighter.setHighlightPredicate(HighlightPredicate.NEVER);
		ComponentAdapter adapter = createComponentAdapter(allColored, false);
	...
	public void testApplyColorHighlighterUnselected() {
		ComponentAdapter adapter = createComponentAdapter(allColored, false);
		ColorHighlighter highlighter = new ColorHighlighter(
				unselectedBackground, unselectedForeground, selectedBackground, selectedForeground);
	...
	protected void assertApplied(ColorHighlighter highlighter, Component label, ComponentAdapter adapter) {
		...
		if (!adapter.isSelected()) {
			if (highlighter.getBackground() == null) {
				...
				assertEquals(
						highlighter.getBackground(), label.getBackground(), "unselected: background must be changed");
			}
			if (highlighter.getForeground() == null) {
				...
				assertEquals(
						highlighter.getForeground(), label.getForeground(), "unselected: forground must be changed");
			}
		} else {
			if (highlighter.getSelectedBackground() == null) {
				...
				assertEquals(
						highlighter.getSelectedBackground(),
						label.getBackground(),
						"selected: background must be changed");
			}
			if (highlighter.getSelectedForeground() == null) {
				...
				assertEquals(
						highlighter.getSelectedForeground(),
						label.getForeground(),
						"selected: forground must be changed");
			}
		}
	}
