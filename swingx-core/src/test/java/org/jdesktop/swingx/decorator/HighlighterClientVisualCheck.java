...
public class HighlighterClientVisualCheck extends InteractiveTestCase {
	...
	public void interactivePatternPredicate() {
		...
		Action action = new AbstractActionExt("togglePredicate") {
			...
			PatternPredicate testAllHighlightAll =
					new PatternPredicate(".*a.*", PatternPredicate.ALL, PatternPredicate.ALL);
			int current;
	...
	public void interactiveToolTipOnTruncatedText() {
		...
		table.getColumn(AncientSwingTeam.COLOR_COLUMN)
				.setCellRenderer(new DefaultTableRenderer(StringValues.TO_STRING, iv));
