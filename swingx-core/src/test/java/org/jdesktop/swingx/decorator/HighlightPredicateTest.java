...
public class HighlightPredicateTest extends InteractiveTestCase {
	...
	public void testOr() {
		...
		HighlightPredicate oneFalseOneTrue =
				new OrHighlightPredicate(HighlightPredicate.NEVER, HighlightPredicate.ALWAYS);
		assertTrue(oneFalseOneTrue.isHighlighted(allColored, adapter));
		HighlightPredicate oneTrueOneFalse =
				new OrHighlightPredicate(HighlightPredicate.ALWAYS, HighlightPredicate.NEVER);
	...
	public void testAnd() {
		...
		assertFalse(oneFalse.isHighlighted(allColored, adapter));
		HighlightPredicate oneFalseOneTrue =
				new AndHighlightPredicate(HighlightPredicate.NEVER, HighlightPredicate.ALWAYS);
		assertFalse(oneFalseOneTrue.isHighlighted(allColored, adapter));
		HighlightPredicate oneTrueOneFalse =
				new AndHighlightPredicate(HighlightPredicate.ALWAYS, HighlightPredicate.NEVER);
	...
	public void testEqualsNotNull() {
		HighlightPredicate predicate = new EqualsHighlightPredicate(allColored.getText());
		ComponentAdapter adapter = createComponentAdapter(allColored, false);
		assertTrue(predicate.isHighlighted(allColored, adapter));
		allColored.setText(null);
		assertFalse(predicate.isHighlighted(allColored, adapter));
	...
	public void testSearchHighlightColumn() {
		...
		ComponentAdapter adapter = createComponentAdapter(allColored, false);
		assertFalse(predicate.isHighlighted(allColored, adapter), "predicate must not match");
		adapter.column = column;
		assertTrue(predicate.isHighlighted(allColored, adapter), "predicate must match");
	}
	...
	public void testSearchHighlightNullString() {
		...
		HighlightPredicate predicate = new SearchPredicate(regex);
		ComponentAdapter adapter = createComponentAdapter(allColored, false);
		assertEquals(false, predicate.isHighlighted(allColored, adapter), "null regex must be no match");
	}
	...
	public void testSearchHighlightEmptyString() {
		...
		HighlightPredicate predicate = new SearchPredicate(regex);
		ComponentAdapter adapter = createComponentAdapter(allColored, false);
		assertEquals(false, predicate.isHighlighted(allColored, adapter), "empty regex must be no match");
	}
	...
	public void testSearchHighlightNullPattern() {
		...
		HighlightPredicate predicate = new SearchPredicate(pattern);
		ComponentAdapter adapter = createComponentAdapter(allColored, false);
		assertEquals(false, predicate.isHighlighted(allColored, adapter), "null pattern must be no match");
	}
	 ...
	 * @return a ComponentAdapter of the label with the given properties.
	 */
	protected ComponentAdapter createComponentAdapter(
			final JLabel label, final boolean selected, final boolean editable) {
