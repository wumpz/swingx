...
public class JXEditorPaneTest extends InteractiveTestCase {
	...
	public void editorCutPaste(JEditorPane editor) {
		...
		cut.actionPerformed(new ActionEvent(editor, 0, (String) cut.getValue(Action.ACTION_COMMAND_KEY)));

		// XXX caret position should be moved in the cut operation
		editor.setCaretPosition(4);

		paste.actionPerformed(new ActionEvent(editor, 0, (String) paste.getValue(Action.ACTION_COMMAND_KEY)));
