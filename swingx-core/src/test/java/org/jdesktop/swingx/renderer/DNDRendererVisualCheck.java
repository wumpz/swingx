...
public class DNDRendererVisualCheck extends InteractiveTestCase {
	...
	private static final Logger LOG = Logger.getLogger(DNDRendererVisualCheck.class.getName());
	...
	public void interactiveDropOnHierachicalColumnBlinks() {
		...
		JXFrame frame = wrapWithScrollingInFrame(xTable, "TreeTable hierarchical: blink on drop over?");
		JTextField textField = new JTextField("drag me over hierarchical column ..");
	...
	public void interactiveDropOnWrappingIconPanelTree() {
		...
		xTable.expandAll();
		JXFrame frame = wrapWithScrollingInFrame(xTable, "Wrapping renderer: blink on drop over?");
	...
	public void interactiveDropOnHyperlinkTableBlinks() {
		...
		installTransferHandler(xTable);
		xTable.getColumn(0).setCellRenderer(new DefaultTableRenderer(new HyperlinkProvider()));
		JXFrame frame = wrapWithScrollingInFrame(xTable, "Hyperlink as renderer: blink on drop over?");
	...
	public void interactiveDropOnWrappingIconPanelTableBlinks() {
		JXTable xTable = new JXTable(new AncientSwingTeam());
		installTransferHandler(xTable);
		xTable.getColumn(0).setCellRenderer(new DefaultTableRenderer(new WrappingProvider()));
		JXFrame frame = wrapWithScrollingInFrame(xTable, "WrappingIconPanel as renderer: blink on drop over?");
	...
	public void interactiveDropOnPlainLabelTableBlinks() {
		...
		JXFrame frame = wrapWithScrollingInFrame(xTable, "Plain label as renderer: blink on drop over?");
		JTextField textField = new JTextField("drag me over first column....");
		textField.setDragEnabled(true);
		addStatusComponent(frame, textField);
		frame.setVisible(true);
	...
	public void interactiveDropOnVisualsTable() {
		...
		JXFrame frame = wrapWithScrollingInFrame(
				xTable, table, "JXTable (first column core renderer) <-> core table: drop-on appearance");
		JTextField textField = new JTextField("drag me over first column....");
		textField.setDragEnabled(true);
		addStatusComponent(frame, textField);
		show(frame);
	...
	public void interactiveDropOnVisualsList() {
		...
		xTable.setDropMode(DropMode.ON_OR_INSERT);
		table.setDropMode(DropMode.ON_OR_INSERT);
		JXFrame frame = wrapWithScrollingInFrame(xTable, table, "JXList <-> core list: drop-on appearance");
		JTextField textField = new JTextField("drag me over first column....");
		textField.setDragEnabled(true);
		addStatusComponent(frame, textField);
		show(frame);
	...
	public void interactiveDropOnVisualsTree() {
		...
		xTable.setDropMode(DropMode.ON_OR_INSERT);
		table.setDropMode(DropMode.ON_OR_INSERT);
		JXFrame frame = wrapWithScrollingInFrame(xTable, table, "JXTree <-> core tree: drop-on appearance");
		JTextField textField = new JTextField("drag me over first column....");
		textField.setDragEnabled(true);
		addStatusComponent(frame, textField);
		show(frame);
