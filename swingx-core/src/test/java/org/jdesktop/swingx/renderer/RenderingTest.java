...
public class RenderingTest extends InteractiveTestCase {
	...
	public void testRenderingComponentNames() throws Exception {
		...
		assertEquals(null, provider.getRendererComponent(null).getName(), "default name expected null");
		...
		try {
			...
			String nameT = tableT.getTableCellRendererComponent(table, null, false, false, 0, 0)
					.getName();
			...
			String nameL = listR.getListCellRendererComponent(list, null, 0, false, false)
					.getName();
			...
			String nameTree = treeR.getTreeCellRendererComponent(tree, null, false, false, false, 1, false)
					.getName();
			assertEquals("Tree.cellRenderer", nameTree, "sanity: checking default name");
			assertEquals(
					nameTree,
					rendererTree
							.getTreeCellRendererComponent(tree, null, false, false, false, 0, false)
							.getName());
	...
	public void testHyperlinkPainterAware() {
		...
		assertTrue(
				provider.getRendererComponent(null) instanceof PainterAware,
				"hyperlink as rendering comp must be PainterAware");
	}
	...
	public void testWrappingIconPanelPainterAware() {
		...
		assertTrue(
				rendererComponent instanceof PainterAware, "wrappingIconPanel as rendering comp must be PainterAware");
	}
	...
	public void testHyperlinkProviderForeground() {
		...
		assertEquals(
				...
				hyperlink.getForeground(),
				"hyperlink foreground set to table's selection foreground");
	}
	...
	public void testWrappingProviderUserObjectUnwrapRespectString() {
		...
		assertEquals(
				...
				provider.getString(context.getValue()),
	...
	public void testToolTipManagerHyperlinkProvider() {
		...
		assertEquals("dummy", label.getToolTipText(), "sanity - tooltip is set to value");
		assertToolTipManagerNotRegistered(label);
	}
	...
	private void assertToolTipManagerNotRegistered(JComponent label) {
		...
		for (MouseListener l : listeners) {
			assertEquals(false, l instanceof ToolTipManager, "registered with tooltipManager");
		}
	}
	...
	public void testLabelProviderRespectIconValueNoString() {
		ImageIcon icon = (ImageIcon) XTestUtils.loadDefaultIcon();
		icon.setDescription("description");
		LabelProvider provider = new LabelProvider(new MappedValue(StringValues.EMPTY, IconValues.ICON));
		TableCellContext context = new TableCellContext();
		context.value = icon;
		JLabel label = provider.getRendererComponent(context);
	...
	public void testLabelProviderRespectStringIconValueBoth() {
		ImageIcon icon = (ImageIcon) XTestUtils.loadDefaultIcon();
		icon.setDescription("description");
		LabelProvider provider = new LabelProvider(new MappedValue(StringValues.TO_STRING, IconValues.ICON));
		TableCellContext context = new TableCellContext();
		context.value = icon;
		JLabel label = provider.getRendererComponent(context);
		assertEquals(icon, label.getIcon());
	...
	public void testWrappingProviderWrappeeConstructors() {
		...
		assertWrappingProviderState(new WrappingProvider(delegate), null, null, delegate, true);
		assertWrappingProviderState(new WrappingProvider(delegate, false), null, null, delegate, false);
		assertWrappingProviderState(new WrappingProvider(iv, delegate, false), iv, null, delegate, false);
		assertWrappingProviderState(
				new WrappingProvider(iv, (ComponentProvider<?>) null, false), iv, null, null, false);
		assertWrappingProviderState(
				new WrappingProvider(null, (ComponentProvider<?>) null, false), null, null, null, false);
	}
	...
	public void testWrappingProviderWrappeeStringValueConstructors() {
		...
		assertWrappingProviderState(new WrappingProvider(iv), iv, null, null, true);
		assertWrappingProviderState(new WrappingProvider(iv, sv), iv, sv, null, true);
		assertWrappingProviderState(new WrappingProvider(sv), null, sv, null, true);
		assertWrappingProviderState(new WrappingProvider(null, sv), null, sv, null, true);
		assertWrappingProviderState(new WrappingProvider(iv, null), iv, null, null, true);
		assertWrappingProviderState(new WrappingProvider(null, null), null, null, null, true);
	...
	public void testWrappingProviderEmptyConstructor() {
		assertWrappingProviderState(new WrappingProvider(), null, null, null, true);
		//        assertWrappingProviderState(new WrappingProvider(false),
		//                null, null, null, false);
	}

	private void assertWrappingProviderState(
			WrappingProvider provider, IconValue iv, StringValue sv, ComponentProvider<?> delegate, boolean unwrap) {
		if (iv == null) {
			assertEquals(StringValues.EMPTY, provider.getStringValue(), "default StringValue must be empty");
		} else {
			assertTrue(
					provider.getStringValue() instanceof MappedValue,
					"provider's StringValue must be a MappedValue containing the IconValue, but" + "was "
							+ provider.getStringValue().getClass());
		...
		if (sv != null) {
			assertEquals(
					sv, provider.getWrappee().getStringValue(), "wrappee's StringValue must be configured to given");
		}
		assertEquals(unwrap, provider.getUnwrapUserObject());
	}
	/**
	...
	public void testDefaultListRendererConstructorWithIconValueAndAlign() {
		DefaultListRenderer renderer =
				new DefaultListRenderer(StringValues.TO_STRING, IconValues.ICON, JLabel.TRAILING);
		assertTrue(renderer.componentController instanceof LabelProvider);
		LabelProvider provider = (LabelProvider) renderer.componentController;
		ImageIcon icon = (ImageIcon) XTestUtils.loadDefaultIcon();
		icon.setDescription("description");
	...
	public void testDefaultTableRendererConstructorWithIconValueAndAlign() {
		DefaultTableRenderer renderer =
				new DefaultTableRenderer(StringValues.TO_STRING, IconValues.ICON, JLabel.TRAILING);
		assertTrue(renderer.componentController instanceof LabelProvider);
		LabelProvider provider = (LabelProvider) renderer.componentController;
		ImageIcon icon = (ImageIcon) XTestUtils.loadDefaultIcon();
		icon.setDescription("description");
