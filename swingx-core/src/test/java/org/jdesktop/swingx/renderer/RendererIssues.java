...
public class RendererIssues extends InteractiveTestCase {
	...
	public static class RolloverCheckBox extends CheckBoxProvider implements RolloverRenderer {
		boolean wasEditable;
	...
	public void interactiveToolTipList() {
		...
		JXList list = new JXList(model) {
			...
			public String getToolTipText(MouseEvent event) {
				...
				if (r == null) return super.getToolTipText(event);
	...
	public void interactiveToolTipTree() {
		...
		JXTree tree = new JXTree(model) {
			...
			private int getVisibleWidth() {
				int width = getVisibleRect().width;
				int indent =
						(((BasicTreeUI) getUI()).getLeftChildIndent() + ((BasicTreeUI) getUI()).getRightChildIndent());
				return width;
			}
	...
	public static class NormalButtonProvider extends CheckBoxProvider implements RolloverRenderer {
		...
		protected void configureState(CellContext context) {
			...
			Point p = (Point) context.getComponent().getClientProperty(RolloverProducer.ROLLOVER_KEY);
			if (
			/* hasFocus || */ (p != null && (p.x >= 0) && (p.x == context.getColumn()) && (p.y == context.getRow()))) {
				rendererComponent.getModel().setRollover(true);
			} else {
				rendererComponent.getModel().setRollover(false);
			}
		...
		protected AbstractButton createRendererComponent() {
			...
			border = BorderFactory.createCompoundBorder(new DropShadowBorder(), button.getBorder());
			return button;
		}
	...
	public void testListFocusBorder() throws UnsupportedLookAndFeelException {
		...
		try {
			...
			JComponent coreComponent =
					(JComponent) coreListRenderer.getListCellRendererComponent(list, null, 0, false, true);
			// sanity: known standard behaviour
			assertEquals(focusBorder, coreComponent.getBorder());
			// prepare extended
			JComponent xComponent = (JComponent) xListRenderer.getListCellRendererComponent(list, null, 0, false, true);
	...
	public void testUpdateRendererOnLFChange() {
		...
		assertEquals(comparisonComponent.getFont(), rendererComp.getFont(), "renderer font must be updated");
	}
