...
public class RendererVisualCheck extends InteractiveTestCase {
	...
	public void interactiveCheckBoxRendererOpacity1513() {
		...
		table.addHighlighter(new PainterHighlighter(
				HighlightPredicate.ROLLOVER_ROW, new MattePainter(PaintUtils.setAlpha(Color.RED, 100))));
		//                new BusyPainter()));
	...
	public void interactiveButtonProvider() {
		...
		table.getColumn(1)
				.setCellRenderer(
						new DefaultTableRenderer(new WrappingProvider(IconValues.NONE, new ButtonProvider(), false) {
	...
	public static class ButtonProvider extends ComponentProvider<JButton> implements RolloverRenderer {
	...
	private Object[] columnNames = {"Buy/Sell", "Type", "SubType", "Ccy1", "Amount1", "Ccy2", "Amount2", "DealId"};
	private Object[][] data = {
	...
	public void interactiveAlternateRowGroupingRobSecond() {
		...
		showWithScrollingInFrame(table, "Rob: value-grouped striping, sort-robust");
	}
	...
	public void interactiveTreeNodeCustomIcons() {
		...
		tree.addHighlighter(
				new BorderHighlighter(HighlightPredicate.ROLLOVER_ROW, BorderFactory.createLineBorder(Color.BLUE)));
	...
	public void interactiveTableSQLDateTime() {
		...
		DefaultTableRenderer renderer = new DefaultTableRenderer(new LabelProvider(SwingConstants.RIGHT));
		...
		table.getColumnExt(4)
				.setCellRenderer(
						new DefaultTableRenderer(new FormatStringValue(DateFormat.getTimeInstance(DateFormat.SHORT))));
	...
	public void interactiveTextAreaRendererTable() {
		...
		model.addRow(new String[] {
			"some really, maybe really really long text -  " + "wrappit .... where needed ",
			"some really, maybe really really long text -  " + "wrappit .... where needed "
		});
		model.addRow(new String[] {
			"another really, maybe really really long text -  " + "with nothing but junk. wrappit .... where needed",
			"some really, maybe really really long text -  " + "wrappit .... where needed "
		});
	...
	public void interactiveTextAreaRendererList() {
		...
		model.addElement(
				"Start: This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. This is a long text. ");
		model.addElement(
				"Start: This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. This is an even longer text. ");
	...
	public void interactiveXLabelRenderer() {
		...
		model.addRow(new String[] {"some really, maybe really really long text -  " + "wrappit .... where needed "});
		model.addRow(new String[] {
			"another really, maybe really really long text -  " + "with nothing but junk. wrappit .... where needed"
		});
		JXTable table = new JXTable(model);
	...
	public void interactiveListDisabledIconRenderer() {
		...
		JXFrame frame =
				wrapWithScrollingInFrame(standard, enhanced, "Disabled - compare renderers: default <--> enhanced");
		addAction(frame, action);
	...
	private TableModel createTableModelWithDefaultTypes() {
		...
		final Class<?>[] types = {
			Object.class, Number.class, Double.class, Date.class, ImageIcon.class, Boolean.class, URI.class
		};
		...
		for (int i = 0; i < 10; i++) {
			...
			try {
				values = new Object[] {
					...
					i,
					Math.random() * 100,
					new Date(today.getTime() + i * 1000000),
					icon,
					i % 2 == 0,
					new URI("http://swingx.java.net")
				};
				model.addRow(values);
	...
	public void interactiveTableCompareFocusedCellBackground() {
		...
		JXFrame frame =
				wrapWithScrollingInFrame(xtable, table, "JTable: Unselected focused background: core/ext renderer");
	...
	public void interactiveXTableCompareFocusedCellBackground() {
		...
		//        JXFrame frame = wrapWithScrollingInFrame(xtable, table, "JXTable: Unselected focused background:
		// core/ext renderer");
		//        getStatusBar(frame).add(new JLabel("different background for unselected lead: first column is
		// not-editable"));
		//        frame.pack();
		//        frame.setVisible(true);
	}
	...
	public void interactiveCheckListColumnControl() {
		...
		list.setCellRenderer(new DefaultListRenderer(provider));
		JXFrame frame = showWithScrollingInFrame(table, list, "rollover checkbox list-renderer");
	...
	public void interactiveTableCustomColorRendererWithHighlighter() {
		TableModel model = new AncientSwingTeam();
		JXTable xtable = new JXTable(model);
		xtable.addHighlighter(HighlighterFactory.createSimpleStriping(HighlighterFactory.GENERIC_GRAY));
		xtable.setDefaultRenderer(Color.class, new ColorRenderer(true));
		JXTable table = new JXTable(model);
		...
		table.setDefaultRenderer(Color.class, renderer);
		JXFrame frame =
				wrapWithScrollingInFrame(xtable, table, "JXTable/highlighter: Custom color renderer - standard/ext");
	...
	public void interactiveTableCustomColorRendererWithHighlighterDontTouch() {
		...
		JXFrame frame = wrapWithScrollingInFrame(
				xtable, table, "JXTable/highlighter dont-touch: Custom color renderer - standard/ext");
	...
	public void interactiveTableAndListCustomColorRenderingController() {
		...
		JXFrame frame = wrapWithScrollingInFrame(
				xtable, list, "JXTable/JXList: Custom color renderer - sharing the component controller");
	...
	public void interactiveTableAndTreeCustomColorRenderingController() {
		...
		JXFrame frame = wrapWithScrollingInFrame(
				xtable, tree, "JXTable/JXTree: Custom color renderer - sharing the component controller");
		...
		ComponentProvider<JLabel> context = new LabelProvider() {
			...
			protected void configureState(CellContext context) {
				...
				if (context.isSelected()) {
					selectedBorder = BorderFactory.createMatteBorder(2, 5, 2, 5, context.getSelectionBackground());
				} else {
					selectedBorder = BorderFactory.createMatteBorder(2, 5, 2, 5, context.getBackground());
				}
	...
	private LinkModel createLinkModel(int i) {
		LinkModel link = null;
		try {
			link = new LinkModel("a link text " + i, null, new URL("http://some.dummy.url" + i));
	...
	private TableModel createTableModelWithURI() {
		String[] columnNames = {"text only", "URI editable", "URI not-editable", "Bool editable", "Bool not-editable"};
