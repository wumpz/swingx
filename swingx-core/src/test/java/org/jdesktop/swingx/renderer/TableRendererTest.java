...
public class TableRendererTest extends InteractiveTestCase {
	...
	private TableModel createTableModelWithDefaultTypes() {
		...
		for (int i = 0; i < 10; i++) {
			Object[] values = new Object[] {
				"row " + i, i, Math.random() * 100, new Date(today.getTime() + i * 1000000), icon, i % 2 == 0
			};
			model.addRow(values);
		}
		return model;
	...
	public void testTableFocusSelectedBorder() {
		...
		JComponent coreComponent =
				(JComponent) coreTableRenderer.getTableCellRendererComponent(table, null, true, true, 0, coreColumn);
		// sanity: known standard behaviour
		assertEquals(selectedFocusBorder, coreComponent.getBorder());
		// prepare extended
		JComponent xComponent =
				(JComponent) xTableRenderer.getTableCellRendererComponent(table, null, true, true, 0, xColumn);
	...
	public void testTableFocusBorder() {
		...
		// need to prepare directly - focus is true only if table is focusowner
		JComponent coreComponent =
				(JComponent) coreTableRenderer.getTableCellRendererComponent(table, null, false, true, 0, coreColumn);
		// sanity: known standard behaviour
		assertEquals(focusBorder, coreComponent.getBorder());
		// prepare extended
		JComponent xComponent =
				(JComponent) xTableRenderer.getTableCellRendererComponent(table, null, false, true, 0, xColumn);
		// assert behaviour same as standard
		assertEquals(coreComponent.getBorder(), xComponent.getBorder());
	}
	/**
	...
	public void testTableRendererExtFocusedNotSelectedEditable() {
		...
		Component coreComponent =
				coreTableRenderer.getTableCellRendererComponent(table, null, false, true, 0, coreColumn);
		// sanity: known standard behaviour
		...
		Component xComponent = xTableRenderer.getTableCellRendererComponent(table, null, false, true, 0, xColumn);
		// assert behaviour same as standard
