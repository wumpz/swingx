...
public class TreeRendererTest extends InteractiveTestCase {
	...
	public void testDelegateFont() {
		...
		WrappingIconPanel component = (WrappingIconPanel)
				tree.getCellRenderer().getTreeCellRendererComponent(tree, "something", false, false, false, 0, false);
	...
	public void interactiveXTreeLabelFormattingHighlighter() {
		...
		final JXFrame frame =
				wrapWithScrollingInFrame(tree, xtree, "custom format - tree vs. xtree (+Rollover renderer)");
		frame.setVisible(true);
	}
	...
	public void interactiveCompareTreeRToL() {
		...
		JTree tree = new JTree();
		tree.setCellRenderer(xTreeRenderer);
		final JXFrame frame = wrapWithScrollingInFrame(xtree, tree, "orientation - core vs. ext renderer");
