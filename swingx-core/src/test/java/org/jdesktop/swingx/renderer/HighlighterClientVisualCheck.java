...
public class HighlighterClientVisualCheck extends InteractiveTestCase {
	...
	public void interactiveBorderHighlighterOnRollover() {
		...
		final AbstractHighlighter hlRow = new BorderHighlighter(
				...
				true,
				false);
		final AbstractHighlighter hlColumn =
				new ColorHighlighter(HighlightPredicate.ROLLOVER_COLUMN, Color.LIGHT_GRAY, null);
	...
	public void interactiveTableBorderHighlighter() {
		...
		BorderHighlighter outer = new BorderHighlighter(
				new HighlightPredicate.ColumnHighlightPredicate(1), BorderFactory.createLineBorder(Color.RED, 3));
		BorderHighlighter inner = new BorderHighlighter(
				new HighlightPredicate.ColumnHighlightPredicate(2),
				BorderFactory.createLineBorder(Color.RED, 3),
				true,
				true);
		BorderHighlighter replace = new BorderHighlighter(
	...
	public void interactiveTablePatternHighlighterJP() {
		...
		AbstractHighlighter gradient =
				createRelativeGradientHighlighter(HorizontalAlignment.LEFT, AncientSwingTeam.INTEGER_COLUMN);
		gradient.setHighlightPredicate(
				new HighlightPredicate.ColumnHighlightPredicate(AncientSwingTeam.INTEGER_COLUMN));
	...
	public void interactiveRolloverRowGroup() {
		...
		final ColorHighlighter highlighter = new ColorHighlighter(HighlightPredicate.NEVER, Color.YELLOW, null);
	...
	public void interactiveColorValueMappedHighlighterPredicate() {
		...
		Color[] colors = new Color[] {Color.YELLOW, Color.CYAN, Color.MAGENTA, Color.GREEN};
		...
		showWithScrollingInFrame(table, "compound highlighter with value-based color mapping predicate");
	}
	...
	public void interactiveColorValueMappedHighlighter() {
		...
		for (int i = 0; i < colors.length; i++) {
			for (int j = 0; j < rowsPerColor; j++) {
				lookup.put((Integer) table.getValueAt(i * rowsPerColor + j, numberColumn), colors[i]);
			}
		}
		table.resetSortOrder();
		...
		showWithScrollingInFrame(table, "conditional highlighter with value-based color mapping");
	}
	...
	public void interactiveSearchPanel() {
		JXTable table = new JXTable(tableModel);
		final ColorHighlighter cl = new ColorHighlighter(new PatternPredicate((Pattern) null, 0), null, Color.RED);
		...
		PatternMatcher patternMatcher = new PatternMatcher() {
			...
			public void setPattern(Pattern pattern) {
				...
				cl.setHighlightPredicate(new PatternPredicate(pattern, old.getTestColumn(), old.getHighlightColumn()));
			}
		...
		JXFrame frame = wrapWithScrollingInFrame(table, "Pattern highlighting col 0");
	...
	public void interactiveTableAlternateAndHyperlink() {
		...
		table.addHighlighter(HighlighterFactory.createSimpleStriping(HighlighterFactory.CLASSIC_LINE_PRINTER));
		JFrame frame = wrapWithScrollingInFrame(table, "classic lineprinter and hyperlink on column 1");
	...
	public void interactiveTablePatternHighlighter() {
		JXTable table = new JXTable(tableModel);
		table.setColumnControlVisible(true);
		table.addHighlighter(new ColorHighlighter(new PatternPredicate("^M", 1), null, Color.red));
	...
	public void interactiveTableCustomRendererColor() {
		...
		showWithScrollingInFrame(
				table, nohighlight, "ext: custom colored renderer with bg highlighter <--> shared without highl");
	}
	...
	public void interactiveValueBasedToolTipAndColorOnHighlighter() {
		...
		ColorHighlighter hl = new ColorHighlighter(predicate, null, Color.RED, null, Color.RED);
		...
		table.setHighlighters(HighlighterFactory.createSimpleStriping(HighlighterFactory.GENERIC_GRAY), hl, tl);
	...
	public void interactiveTableColorBasedOnComponentValue() {
		...
		table.addHighlighter(hl);
		showWithScrollingInFrame(table, "component value-based rendering (label text contains y) ");
	}
