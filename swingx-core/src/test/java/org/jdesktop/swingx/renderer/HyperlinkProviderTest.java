...
public class HyperlinkProviderTest extends InteractiveTestCase {
	...
	public void testMixedValueTypes() {
		...
		TableCellRenderer linkRenderer = new DefaultTableRenderer(new HyperlinkProvider(action, LinkModel.class));
	...
	public void testSubclassedValueTypes() {
		LinkModelAction<?> action = new LinkModelAction<>(new EditorPaneLinkVisitor());
		TableCellRenderer renderer = new DefaultTableRenderer(new HyperlinkProvider(action, LinkModel.class));
	...
	public void testSelectionBackground() {
		...
		JXHyperlink hyperlink =
				(JXHyperlink) linkRenderer.getTableCellRendererComponent(table, link, true, false, 1, 0);
		...
		assertEquals(
				...
				hyperlink.getBackground(),
				"background must be table selection background");
		hyperlink = (JXHyperlink) linkRenderer.getTableCellRendererComponent(table, link, false, false, 1, 0);
		assertEquals(table.getBackground(), hyperlink.getBackground(), "background must be table background");
	}
	...
	public void testRolloverRecognition() {
		...
		table.getColumnModel().getColumn(0).setCellRenderer(linkRenderer);
		JXHyperlink hyperlink =
				(JXHyperlink) linkRenderer.getTableCellRendererComponent(table, link, false, false, 1, 0);
		assertFalse(hyperlink.getModel().isRollover(), "renderer must not be rollover");
		table.putClientProperty(RolloverProducer.ROLLOVER_KEY, new Point(0, 1));
		hyperlink = (JXHyperlink) linkRenderer.getTableCellRendererComponent(table, link, false, false, 1, 0);
		assertTrue(hyperlink.getModel().isRollover(), "renderer must be rollover");
	}
