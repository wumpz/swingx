...
public class CompoundFocusListenerTest extends InteractiveTestCase {
	...
	private static final Logger LOG = Logger.getLogger(CompoundFocusListenerTest.class.getName());
	...
	private void assertRemoved(PropertyChangeReport report, PropertyChangeListener[] propertyChangeListeners) {
	...
	public void testRelease() throws InterruptedException, InvocationTargetException {
		...
		Component focusOwner =
				KeyboardFocusManager.getCurrentKeyboardFocusManager().getPermanentFocusOwner();
		assertFalse(
				SwingXUtilities.isDescendingFrom(focusOwner, picker), "sanity: initial focus must not be in picker");
		CompoundFocusListener l = new CompoundFocusListener(picker);
		final PropertyChangeReport report = new PropertyChangeReport();
		l.addPropertyChangeListener(report);
		l.release();
