...
public class JXMonthViewTest extends InteractiveTestCase {
	...
	public void testZoomableNameByLocaleProvider() {
		...
		JXMonthView monthView = new JXMonthView();
		AbstractHyperlinkAction<?> action =
				(AbstractHyperlinkAction<?>) monthView.getActionMap().get("zoomOut");
		assertSame(monthView, action.getTarget());
		monthView.setLocale(serbianLatin);
		String[] monthNames =
				DateFormatSymbols.getInstance(monthView.getLocale()).getMonths();
	...
	public void testUpdateTodayInAddNotify() {
		...
		assertEquals(
				CalendarUtils.startOfDay(monthView.getCalendar(), clock.getCurrentDate()),
				monthView.getToday(),
				"sanity: monthView takes clock current initially");
		// increment clock
		clock.nextDay();
		// realize monthView
		...
		assertEquals(
				CalendarUtils.startOfDay(monthView.getCalendar(), clock.getCurrentDate()),
				monthView.getToday(),
				" monthView must update today in addNotify");
	}
	...
	public void testZoomableNameOnMonthChange() {
		...
		assertTrue(
				...
				"name must be updated, expected: " + monthNames[nextMonth] + " was: " + action.getName());
	}
	...
	public void testDayForegroundBeforeSunday() {
		assertThrows(IllegalArgumentException.class, () -> monthView.setDayForeground(Calendar.SUNDAY - 1, Color.RED));
	}
	...
	public void testDayForegroundAfterSaturday() {
		assertThrows(
				IllegalArgumentException.class, () -> monthView.setDayForeground(Calendar.SATURDAY + 1, Color.RED));
	}
	...
	public void testDaysOfWeekReset() {
		...
		assertEquals(
				Arrays.asList(uiDays),
				Arrays.asList(monthView.getDaysOfTheWeek()),
				"use ui-provided daysOfWeek after reset");
	}
	...
	public void testCalendarsContructorUnchangedFirstDayOfWeek() {
		...
		assertEquals(first, model.getFirstDayOfWeek(), "model's calendar properties must be unchanged: minimalDays");
		// sanity: taken in monthView
		assertEquals(first, monthView.getFirstDayOfWeek(), "monthView's calendar properties must be synched");
	}
	/**
	 * Issue #733-swingx: model and monthView cal not synched.
	 *
	...
	public void testCalendarsSetModel() {
		...
		// Issue 1143
		assertTrue(
				CalendarUtils.isStartOfDay(monthView.getCalendar()),
				"firstDisplayedDay must be updated to start of day");
		...
		if (firstDayOfWeek != model.getFirstDayOfWeek()) {
			assertEquals(
					...
					monthView.getFirstDayOfWeek(),
					"firstDayOfWeek must be updated from model");
		} else {
		...
		assertEquals(modelMinimal, model.getMinimalDaysInFirstWeek(), "model minimals must not be changed");
	}
	...
	public void testCalendarsContructorUnchangedMinimalDaysOfModel() {
		...
		JXMonthView monthView = new JXMonthView(new Date(), model);
		assertEquals(
				first, model.getMinimalDaysInFirstWeek(), "model's calendar properties must be unchanged: minimalDays");
	}
	...
	public void testCalendarsSetModelUnchangedMinimalDaysInFirstWeek() {
		...
		model.setMinimalDaysInFirstWeek(first);
		monthView.setSelectionModel(model);
		assertEquals(first, model.getMinimalDaysInFirstWeek(), "model minimals must not be changed");
	}
	...
	public void testCalendarsLocaleChangedMonthView() {
		...
		monthView.setLocale(locale);
		assertEquals(
				locale, monthView.getSelectionModel().getLocale(), "locale set in monthView must be passed to model");
	}
	...
	public void testCalendarsLocaleChangedModel() {
		...
		assertEquals(locale, monthView.getLocale(), "locale set in model must be passed to monthView");
	}
	...
	public void testCalendarsLocaleContructor() {
		...
		assertEquals(
				locale,
				monthView.getSelectionModel().getLocale(),
				"initial locale in constructor must be passed to model");
	}
	...
	public void testCalendarsFirstDayOfWeekInitial() {
		JXMonthView monthView = new JXMonthView();
		assertEquals(
				monthView.getFirstDayOfWeek(), monthView.getSelectionModel().getFirstDayOfWeek());
	}
	/**
	 * Issue #733-swingx: model and monthView cal not synched.
	 *
	...
	public void testMonthViewSameAsSelectionModelSelectedDate() {
		...
		Date date = calendar.getTime();
		monthView.setSelectionDate(date);
		assertEquals(monthView.getSelectionDate(), monthView.getSelectionModel().getFirstSelectionDate());
	}
	...
	public void testSelectionModelSameAsMonthViewSelectedDate() {
		...
		Date date = calendar.getTime();
		monthView.getSelectionModel().setSelectionInterval(date, date);
		assertEquals(monthView.getSelectionModel().getFirstSelectionDate(), monthView.getSelectionDate());
	}
	...
	public void testAutoScrollOnSelectionRevalidate() throws InterruptedException, InvocationTargetException {
		...
		SwingUtilities.invokeAndWait(new Runnable() {
			...
			public void run() {
				...
				frame.validate();
				assertEquals(first, us.getFirstDisplayedDay(), "firstDisplayed must not be changed on revalidate");
				//                assertEquals(first, us.getFirstDisplayedDate());
				//                fail("weird (threading issue?): the firstDisplayed is changed in layoutContainer - not
				// testable here");
			}
		});
	}
	...
	public void testTodaySetNotification() {
		...
		CalendarUtils.startOfDay(calendar);
		TestUtils.assertPropertyChangeEvent(report, "today", today, calendar.getTime());
	}
	...
	public void testEnsureVisibleDateNofication() {
		...
		TestUtils.assertPropertyChangeEvent(report, "firstDisplayedDay", firstDisplayedDate, calendar.getTime(), false);
	}
	...
	public void testMonthViewCalendarInvariantOnSetSelection() {
		...
		Date first = monthView.getFirstDisplayedDay();
		assertEquals(
				first, monthView.getCalendar().getTime(), "monthViews calendar represents the first day of the month");
		...
		assertEquals(first, monthView.getCalendar().getTime(), "selection must not change the calendar");
	}
	...
	public void testMonthViewCalendarInvariantOnQuerySelectioon() {
		...
		assertEquals(first, monthView.getCalendar().getTime(), "query selection must not change the calendar");
	}
	...
	public void testMonthViewCalendarInvariantOnSetFirstDisplayedDate() {
		...
		assertEquals(
				next, monthView.getCalendar().getTime(), "monthViews calendar represents the first day of the month");
	}
	...
	public void testEnsureDateVisibleDateParamNextMonth() {
		...
		assertEquals(temp.getTime(), monthView.getFirstDisplayedDay(), "must be scrolled to next month");
	}
	...
	public void testTimeZoneChangeResetUnselectableDates() {
		...
		assertEquals(
				0,
				monthView.getSelectionModel().getUnselectableDates().size(),
				"unselectable dates must have been cleared");
	}
	...
	public void testTimeZoneChangeAnchorInvariant() {
		...
		assertEquals(anchor, monthView.getAnchorDate(), "anchor must be invariant to timezone change");
	}
	...
	public void testTimeZoneChangeNotification() {
		...
		TestUtils.assertPropertyChangeEvent(report, "timeZone", timezone, newTimeZone, false);
	}
	...
	public void testComponentInputMapEnabled() {
		JXMonthView monthView = new JXMonthView();
		assertFalse(monthView.isComponentInputMapEnabled(), "the default value must be false");
		PropertyChangeReport report = new PropertyChangeReport();
		monthView.addPropertyChangeListener(report);
		monthView.setComponentInputMapEnabled(true);
		TestUtils.assertPropertyChangeEvent(report, "componentInputMapEnabled", false, true, false);
		report.clear();
		monthView.setComponentInputMapEnabled(false);
		TestUtils.assertPropertyChangeEvent(report, "componentInputMapEnabled", true, false, false);
	}
	...
	public void testCommitCancelResetsAdjusting() {
		...
		assertFalse(monthView.getSelectionModel().isAdjusting(), "commit must reset adjusting");
		monthView.getSelectionModel().setAdjusting(true);
		monthView.cancelSelection();
		assertFalse(monthView.getSelectionModel().isAdjusting(), "cancel must reset adjusting");
	}
	/**
	 * Enhanced commit/cancel.
	 *
	...
	public void testCommitCancelSameAsOld() {
		JXMonthView picker = new JXMonthView();
		assertSame(
				picker.getActionMap().get("cancelSelection"),
				picker.getActionMap().get(JXMonthView.CANCEL_KEY));
		assertSame(
				picker.getActionMap().get("acceptSelection"),
	...
	public void testAdjustingResetOnCancel() {
		...
		assertFalse(view.getSelectionModel().isAdjusting(), "ui keyboard action must have stopped model adjusting");
	...
	public void testAdjustingSetOnSelect() {
		...
		Action select = view.getActionMap().get("selectNextDay");
		select.actionPerformed(null);
		assertTrue(view.getSelectionModel().isAdjusting(), "ui keyboard action must have started model adjusting");
		assertEquals(2, report.getEventCount());
	...
	public void testUnselectableDate() {
		...
		assertTrue(monthView.isUnselectableDate(today), "raqw today must be unselectable");
		assertTrue(monthView.isUnselectableDate(startOfDay(today)), "start of today must be unselectable");
		assertTrue(monthView.isUnselectableDate(endOfDay(today)), "end of today must be unselectable");
	...
	public void testSelectionModes() {
		JXMonthView monthView = new JXMonthView();
		assertEquals(SelectionMode.SINGLE_SELECTION, monthView.getSelectionMode());
	...
	public void testFlaggedDateRemoveNotify() {
		...
		TestUtils.assertPropertyChangeEvent(report, "flaggedDates", oldFlagged, monthView.getFlaggedDates());
	}
