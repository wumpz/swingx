...
public class DefaultSortControllerTest extends InteractiveTestCase {
	...
	public void testXTableRemoveSelected() {
		...
		assertEquals(table.getRowCount() - 1, table.getSelectedRow(), "last row must be still selected");
	}
	...
	public void testListRemoveSelected() {
		...
		assertEquals(list.getElementCount() - 1, list.getSelectedIndex(), "last row must be still selected");
	}
