...
public class TableSortControllerTest extends AbstractTestSortController<TableSortController<TableModel>, TableModel> {
	...
	private static final Logger LOG = Logger.getLogger(TableSortControllerTest.class.getName());
	...
	public void testComparableComparatorTable() {
		...
		TableSortController<TableModel> sorter = new TableSortController<>(model);
	 ...
	 * @param teamModel2
	 */
	private void initColumnClasses(StringValueRegistry registry, TableModel model) {
	 ...
	 */
	@SuppressWarnings("unused")
	private void installPerClass(StringValueRegistry registry, Class<?>... clazz) {
		Map<Integer, Class<?>> classPerColumn = new HashMap<>();
	...
	@Override
	protected TableSortController<TableModel> createDefaultSortController(TableModel model) {
