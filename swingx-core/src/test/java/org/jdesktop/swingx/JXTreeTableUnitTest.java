...
public class JXTreeTableUnitTest extends InteractiveTestCase {
	...
	public void testRowSorterNull() {
		JXTreeTable table = new JXTreeTable();
		assertNull(table.getRowSorter(), "null rowsorter initially, was: " + table.getRowSorter());
	}
	/**
	 * Issue #1121: most not be sortable
	 *
	...
	public void testRowSorterNotSettable() {
		...
		assertNull(table.getRowSorter(), "rowsorter not settable, was: " + table.getRowSorter());
	}
	/**
	...
	public void testDisabledTreeColumnPrepared() {
		...
		assertEquals(
				false,
				treeTable.prepareRenderer(treeTable.getCellRenderer(0, 0), 0, 0).isEnabled());
	}
	...
	public void testIconSetOnRendererTrueOverwrite() {
		...
		treeTable.setTreeCellRenderer(renderer);
		assertEquals(null, renderer.getLeafIcon(), "renderer leaf icon must be overwritten by treeTable's leaf icon");
	}
	...
	public void testIconOverwriteInitial() {
		JXTreeTable treeTable = new JXTreeTable();
		assertFalse(treeTable.isOverwriteRendererIcons(), "initial overwriteRendererIcons must be false");
	}
	...
	public void testEditCellAtByKeyEvent() {
		...
		// sanity
		assertTrue(treeTable.isCellEditable(0, 0));
		KeyEvent e = new KeyEvent(treeTable, KeyEvent.KEY_PRESSED, 0L, 0, KeyEvent.VK_T, KeyEvent.CHAR_UNDEFINED);
	...
	public void testTableEditCellAtByKeyEvent() {
		...
		KeyEvent e = new KeyEvent(table, KeyEvent.KEY_PRESSED, 0L, 0, KeyEvent.VK_T, KeyEvent.CHAR_UNDEFINED);
	...
	public void testTableEventUpdateOnTreeTableModelSetValue() throws InterruptedException, InvocationTargetException {
		...
		final TableModelReport report = new TableModelReport();
		table.getModel().addTableModelListener(report);
		model.setValueAt("games", table.getPathForRow(6).getLastPathComponent(), 0);
		SwingUtilities.invokeAndWait(new Runnable() {
			@Override
			public void run() {
				assertEquals(1, report.getEventCount(), "tableModel must have fired");
	...
	public void testComponentAdapterIsLeaf() {
		...
		assertEquals(
				model.isLeaf(label),
				table.getComponentAdapter(lastRow, 0).isLeaf(),
				"adapter must report same leafness as model");
		...
		assertEquals(
				model.isLeaf(inner),
				table.getComponentAdapter(firstRow, 0).isLeaf(),
				"adapter must report same leafness as model");
	}
	...
	public void testEditOnCollapse() {
		...
		TreeTableModel model = new DefaultTreeTableModel(root, columnNames) {
			...
			public void setValueAt(Object value, Object node, int col) {
				...
				modelSupport.fireChildChanged(new TreePath(getPathToRoot(parent)), parent.getIndex(treeNode), treeNode);
			}
	...
	public void testAdjustedRowHeights() {
		...
		assertEquals(treeTable.getRowHeight(), tree.getRowHeight(), "table and tree rowHeights must be equal");
	...
	public void testToggleClickCount() {
		...
		assertEquals(newClickCount, treeTable.getToggleClickCount(), "toggleClickCount must be changed");
	...
	public void testDragHackFlagOn() {
		...
		treeTable
				...
				.expandOrCollapseNode(
						0,
						new MouseEvent(
								treeTable, MouseEvent.MOUSE_PRESSED, 0, InputEvent.BUTTON1_MASK, 0, 0, 1, false));
	...
	public static class InsertTreeTableModel extends DefaultTreeTableModel {
		...
		public DefaultMutableTreeTableNode addChild(DefaultMutableTreeTableNode parent) {
			...
			parent.add(newNode);
			modelSupport.fireChildAdded(new TreePath(getPathToRoot(parent)), parent.getIndex(newNode), newNode);
	...
	public void testTreeTableModelIsBoundProperty() {
		...
		assertEquals(1, treeTMPropertyCount, "treeTable must have fired exactly one event for property treeTableModel");
		assertEquals(
				allPropertyCount,
				treeTMPropertyCount,
				"treeTable must have fired event for property treeTableModel only");
	...
	public void testRemoveAllColumsAfterModelChanged() {
		...
		assertEquals(table.getColumnCount(), table.getColumnCount(true), "visible columns must be same as total");
	}
	...
	public void testTableRowAtNegativePoint() {
		...
		assertEquals(
				-1,
				treeTable.rowAtPoint(new Point(-1, negativeYRowHeight)),
				"negative y location rowheight " + negativeYRowHeight + " must return row -1");
		assertEquals(
				-1,
				treeTable.rowAtPoint(new Point(-1, negativeYRowHeightPlusOne)),
				"negative y location " + negativeYRowHeightPlusOne + " must return row -1");
		assertEquals(
				-1,
				treeTable.rowAtPoint(new Point(-1, negativeYMinimal)),
				"minimal negative y location must return row -1");
	}
	...
	public void testPathForLocationContract() {
		...
		// assertEquals("location outside must return null path", null, treeTable.getPathForLocation(-1,
		// -(treeTable.getRowHeight() - 1)));
		int negativeYRowHeight = -treeTable.getRowHeight();
		int negativeYRowHeightPlusOne = negativeYRowHeight + 1;
		int negativeYMinimal = -1;
		assertEquals(
	...
	public void testRendererProperties() {
		...
		assertEquals(expandsSelected, treeTable.getExpandsSelectedPaths(), "expand selected");
		assertEquals(scrollsOnExpand, treeTable.getScrollsOnExpand(), "scrolls expand");
		assertEquals(showRootHandles, treeTable.getShowsRootHandles(), "shows handles");
	...
	public void testLineStyle() {
		...
		JXTree renderer = (JXTree) treeTable.getCellRenderer(0, 0);
		assertEquals("Horizontal", renderer.getClientProperty(propertyName), propertyName + " set on renderer");
	}
	...
	public void testArbitraryClientProperty() {
		...
		treeTable.putClientProperty(propertyName, "Horizontal");
		JXTree renderer = (JXTree) treeTable.getCellRenderer(0, 0);
		assertNull(renderer.getClientProperty(propertyName), propertyName + " not set on renderer");
	}
