...
public class TreeUtilitiesTest extends InteractiveTestCase {
	 ...
	 */
	private void assertSameEnumeration(String message, Enumeration<?> coreEnum, Enumeration<?> xEnum) {
	...
	public static MutableTreeNode createTreeNode(String text, int depth) {
		...
		if (--depth >= 0) for (int i = bellCurve(); --i >= 0; ) node.add(createTreeNode(createCellValue(), depth));
		return node;
	}
	...
	private static String createCellValue() {
		...
		for (int i = c.length; --i >= 0; ) c[i] = (char) (random.nextInt(26) + 'a');
