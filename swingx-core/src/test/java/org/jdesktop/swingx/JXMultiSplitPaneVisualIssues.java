...
public class JXMultiSplitPaneVisualIssues extends InteractiveTestCase {
	...
	public void interactiveTabInCenter() throws IOException {
		String layout = "(ROW " + "(LEAF name=selector weight=0.2)"
		...
		final DocumentListener docL = new DocumentListener() {
			...
			private void updatePreview() {
				editor.setText(area.getText());
				LOG.info("initial size: pref/min/max \n " + pane.getPreferredSize() + pane.getMinimumSize()
						+ pane.getMaximumSize());
			}
	...
	public void interactiveSplitPaneRevalidate() {
		String layout = "(ROW " + "(LEAF name=selector weight=0.3)"
				+ "(COLUMN weight=0.7 "
	...
	private static void prepare(JFrame f) {
		prepare(
				f,
				MultiSplitLayout.parseModel("(ROW (COLUMN weight=0.5 left.top left.bottom)" + " (LEAF name=middle)"
	...
	private static void prepare(JFrame f, Node model) {
		...
		header.setDescription(
				"<html>This is a bit tricky ... To reproduce the issue move one of the dividers and then click reset. Expected behavior is that original layout is restored. Actual behavior (bug) is that only two cells stay displayed and no divider is visible.");
		...
		JButton b = new JButton(new AbstractAction("Reset") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				msp.setModel(MultiSplitLayout.parseModel("(ROW (COLUMN weight=0.5 left.top left.bottom)"
						+ " (LEAF name=middle)" + " (COLUMN weight=0.5 right.top right.bottom))"));
				msp.revalidate();
				msp.repaint();
			}
		});
	...
	private static final Logger LOG = Logger.getLogger(JXMultiSplitPaneVisualIssues.class.getName());
}
