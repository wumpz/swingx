...
public class JXListTest extends InteractiveTestCase {
	...
	public void testNextMatchUseString() {
		...
		assertEquals(-1, list.getNextMatch("b", 0, Bias.Forward), "must not find a match for 'b', all start with 'r'");
	}
	/**
	...
	public void testRevalidateOnSetRowFilter() throws InterruptedException, InvocationTargetException {
		...
		SwingUtilities.invokeAndWait(new Runnable() {
			@Override
			public void run() {
				RowFilter<? super ListModel, ? super Integer> filter =
						RowFilters.regexFilter(Pattern.CASE_INSENSITIVE, "^b");
		...
		SwingUtilities.invokeAndWait(new Runnable() {
			...
			public void run() {
				...
				assertTrue(
						...
						"height must be adjusted to reduced number of rows, " + "but was (old/current): " + size.height
								+ "/" + list.getSize().height);
			}
		});
	}
	...
	public void testSortOrderCycle() {
		...
		TestUtils.assertPropertyChangeEvent(
				report, "sortOrderCycle", DefaultSortController.getDefaultSortOrderCycle(), list.getSortOrderCycle());
	}
	...
	public void testComponentAdapterCoordinates() {
		...
		list.toggleSortOrder();
		// sanity asssert
		assertEquals(list.getElementAt(0), originalFirstRowValue, "view order must be unchanged ");
		// invert sort
		list.toggleSortOrder();
		// sanity assert
		assertEquals(list.getElementAt(0), originalLastRowValue, "view order must be reversed changed ");
		ComponentAdapter adapter = list.getComponentAdapter();
		assertEquals(
				list.getElementAt(0),
		...
		adapter.column = 0;
		assertEquals(
				list.getElementAt(0), adapter.getValue(), "adapter.getValue must return value at adapter coordinates");
		assertEquals(
				list.getElementAt(0), adapter.getValue(0), "adapter.getValue must return value at adapter coordinates");
	}
	...
	public void testWrappedRendererDefault() {
		...
		assertSame(
				renderer.getDelegateRenderer(),
				list.getWrappedCellRenderer(),
				"wrapping renderer must use list's default on null");
	}
	...
	public void testDelegatingRendererUseDefaultSetNull() {
		...
		assertEquals(
				...
				renderer.getDelegateRenderer().getClass(),
				"wrapping renderer must use list's default on null");
	}
	...
	public void testDelegatingRendererUseDefault() {
		JXList list = new JXList();
		ListCellRenderer defaultRenderer = list.createDefaultCellRenderer();
		assertEquals(DefaultListRenderer.class, defaultRenderer.getClass(), "sanity: creates default");
		DelegatingRenderer renderer = (DelegatingRenderer) list.getCellRenderer();
	...
	public void testDelegatingRendererUseCustomDefault() {
		...
		ListCellRenderer defaultRenderer = list.createDefaultCellRenderer();
		assertEquals(CustomDefaultRenderer.class, defaultRenderer.getClass(), "sanity: creates custom");
		DelegatingRenderer renderer = (DelegatingRenderer) list.getCellRenderer();
		assertEquals(defaultRenderer.getClass(), renderer.getDelegateRenderer().getClass());
	}
	/**
