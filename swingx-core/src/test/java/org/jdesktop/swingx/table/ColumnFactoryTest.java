...
public class ColumnFactoryTest extends InteractiveTestCase {
	...
	public void testPackWithPrepareRenderer() {
		...
		assertEquals(
				table.prepareRenderer(table.getCellRenderer(0, 0), 0, 0).getPreferredSize().width,
	...
	public void testTableCopeWithCreateNullColumn() {
		...
		assertEquals(
				...
				"factory must have created one less than model columns");
	}
	...
	public void testPackColumnWithMax() {
		...
		assertEquals(max, columnExt.getPreferredWidth(), "pref width must be bounded by max");
	}
	/**
	...
	public void testPackColumnIndexToHeaderRenderer() {
		...
		int realPrefWidth = 2 * 4 // magic value - JXTable's default margin,
				...
				+ renderer.getTableCellRendererComponent(table, columnExt.getHeaderValue(), false, false, -1, special)
						.getPreferredSize()
						.width;
