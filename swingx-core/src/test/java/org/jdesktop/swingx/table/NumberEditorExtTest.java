...
public class NumberEditorExtTest extends InteractiveTestCase {
	...
	private static final Logger LOG = Logger.getLogger(NumberEditorExtTest.class.getName());
	...
	public void testStrictIsDefault() {
		...
		assertEquals(true, ((NumberEditorExt) editor).hasStrictFormatter(), "default formatter is strict");
	}
	...
	public void testEditorValueParsing() {
		JFormattedTextField field =
				(JFormattedTextField) cellEditor.getTableCellEditorComponent(table, 100, false, 0, INTEGER_COLUMN);
		// add valid digit
	...
	public void testEditorStrictValueParsing() {
		JFormattedTextField field = (JFormattedTextField)
				cellEditorStrict.getTableCellEditorComponent(table, 100, false, 0, INTEGER_COLUMN);
		// add valid digit
		field.setText(field.getText() + "9");
	...
	public void testEditorValueExceedBounds() {
		JFormattedTextField field = (JFormattedTextField)
				cellEditor.getTableCellEditorComponent(table, Integer.MAX_VALUE, false, 0, INTEGER_COLUMN);
	...
	public void testEditorStrictValueExceedBounds() {
		JFormattedTextField field = (JFormattedTextField)
				cellEditorStrict.getTableCellEditorComponent(table, Integer.MAX_VALUE, false, 0, INTEGER_COLUMN);
		// add valid digit - but exceeds Integer bounds so must not return true
		field.setText(field.getText() + "9");
	...
	public void interactiveStrictNumberEditorAllTypesAsNumber() {
		...
		DefaultTableModel model =
				new DefaultTableModel(
						new String[] {
							"Byte", "Short", "Integer", "Float", "Double", "BigInteger", "BigDecimal", "Number"
						},
						10) {
	...
	public void interactiveFloatingPointEditorDigits() {
		...
		DefaultTableModel model =
				new DefaultTableModel(
						new String[] {
							"Double-defaultstrict", "Double-customMaxDigits-nonstrict", "Double-customMaxDigitsstrict"
						},
						10) {
	...
	public void setUp() throws Exception {
		DefaultTableModel model = new DefaultTableModel(5, COLUMN_COUNT) {
			...
			@Override
			public Class<?> getColumnClass(int columnIndex) {
				if (columnIndex == INTEGER_COLUMN) return Integer.class;
				if (columnIndex == NUMBER_COLUMN) return Number.class;
