...
public class ColumnControlButtonTest extends InteractiveTestCase {
	...
	public void testAdditonalActionGrouping() {
		...
		assertEquals(
				table.getColumnCount()
						+ 1 /* separator */
						+ 3 /*default actions with column. prefix*/
						+ 1 /* separator custom group */
						+ 1,
	...
	public void testAdditionalActionVisible() {
		...
		DefaultColumnControlPopup popup = (DefaultColumnControlPopup) button.getColumnControlPopup();
		assertEquals(
				table.getColumnCount() + 1 /* separator */ + 3,
				popup.getPopupMenu().getComponentCount(),
				"additional actions visible, component count expected ");
	...
	public void testColumnVisibilityActionOnHideableInitial() {
		...
		ColumnVisibilityAction action =
				columnControl.getColumnVisibilityActions().get(0);
	...
	public void testColumnControlIconNotUpdateNonActionUIResource() {
		...
		assertSame(icon, control.getIcon(), "icon must not be updated on LF change if not UIResource: ");
	}
	...
	public void testColumnControlReleaseAction() {
		...
		assertEquals(0, priorityColumn.getPropertyChangeListeners().length, "all listeners must be removed");
	}
	...
	public void testSetLastColumnMenuItemToUnselected() {
		...
		wrapWithScrollingInFrame(table, "");
		ColumnControlButton columnControl = (ColumnControlButton) table.getColumnControl();
		Component[] items = ((DefaultColumnControlPopup) columnControl.getColumnControlPopup())
				.getPopupMenu()
				.getComponents();
	...
	public void testColumnControlInvisibleColumns() {
		...
		assertEquals(totalColumnCount, columnMenuItems, "menu items must be equal to columns");
		JCheckBoxMenuItem menuItem =
				(JCheckBoxMenuItem) ((DefaultColumnControlPopup) columnControl.getColumnControlPopup())
						.getPopupMenu()
						.getComponent(0);
		// sanit assert
		assertEquals(priorityColumn.getHeaderValue(), menuItem.getText());
		assertEquals(
				priorityColumn.isVisible(),
				menuItem.isSelected(),
				"selection of menu must be equal to column visibility");
	}
