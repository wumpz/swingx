...
public class TableColumnModelTest extends InteractiveTestCase {
	...
	public void testHideTableColumnPropertyNotification() {
		...
		TestUtils.assertPropertyChangeEvent(report.getPropertyChangeReport(), "visible", true, false);
	}
	...
	public void testIsAddedFromInvisibleTrue() {
		...
		TableColumnModelListener report = new TableColumnModelListener() {
			...
			public void columnAdded(TableColumnModelEvent e) {
				...
				assertEquals(true, columnModel.isAddedFromInvisibleEvent(fromIndex), "old column really removed");
	...
	private void assertIsRemovedToInvisible(
			final DefaultTableColumnModelExt columnModel, final int index, final boolean expected) {
		// can't use report: the isRemovedToInvisible is valid during notification only
		TableColumnModelListener report = new TableColumnModelListener() {
			...
			public void columnRemoved(TableColumnModelEvent e) {
				...
				assertEquals(expected, columnModel.isRemovedToInvisibleEvent(fromIndex), "moved to invisible");
			}
	...
	public void testShowTableColumnPropertyNotification() {
		...
		verify(l).columnPropertyChange(argThat(property("visible", false, true)));
	}
	...
	public void testHiddenTableColumnPropertyNotification() {
		...
		TestUtils.assertPropertyChangeEvent(report.getPropertyChangeReport(), "identifier", identifier, title);
	}
	...
	public void testTotalColumnWidth() {
		...
		assertEquals(
				...
				"new total width must be old minus invisible column width " + columnWidth);
	}
