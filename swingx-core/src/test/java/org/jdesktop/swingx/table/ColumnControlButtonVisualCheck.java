...
public class ColumnControlButtonVisualCheck extends InteractiveTestCase {
				...
				ColumnControlPopup controlPopup = new DefaultColumnControlPopup() {
					...
					public void addAdditionalActionItems(List<? extends Action> actions) {
						...
						for (Action action : actions) {
							commands += action.getValue(Action.ACTION_COMMAND_KEY);
						}
						LOG.info(commands);
					}
				};
	...
	public void interactiveNullVisibilityAction() {
		...
		table.setColumnControlVisible(true);
		JXFrame frame = wrapWithScrollingInFrame(table, "first model column not hideable - needs custom control");
		frame.setVisible(true);
	}
	...
	public void interactiveTestToggleTableModel() {
		...
		Action toggleAction = new AbstractAction("Toggle TableModel") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				table.setModel(model.equals(tableModel) ? new AncientSwingTeam() : tableModel);
			}
		};
	...
	public void interactiveTestColumnControlInvisibleColumns() {
		...
		JXFrame frame = wrapWithScrollingInFrame(
				table, "ColumnControl (#192, #38-swingx) first enable ColumnControl then column invisible");
	...
	public void interactiveTestColumnControlEarlyInvisibleColumns() {
		...
		JFrame frame = wrapWithScrollingInFrame(
				table, "ColumnControl (#192, #38-swingx) first column invisible, the enable columnControl");
		frame.setVisible(true);
	}
	...
	public void interactiveTestColumnControlSetModelToggleInvisibleColumns() {
		...
		table.setColumnControlVisible(true);
		JXFrame frame = wrapWithScrollingInFrame(
				table, "ColumnControl (#212-swingx) setModel and toggle first column invisible");
		frame.setVisible(true);
	...
	public void interactiveTestLastVisibleColumn() {
		...
		JFrame frame =
				wrapWithScrollingInFrame(table, "JXTable (#192, #38-swingx) ColumnControl and Visibility of items");
		table.getColumnExt(0).setVisible(false);
		frame.setVisible(true);
	}
