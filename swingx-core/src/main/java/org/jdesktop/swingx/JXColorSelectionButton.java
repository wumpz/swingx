...
public class JXColorSelectionButton extends JButton {
	...
	protected void paintComponent(Graphics g) {
		...
		final Color FILL_COLOR = isEnabled()
		...
		if (OS.isMacOSX() && colorwell != null) {
			...
			g.drawRect(
					ins.left, ins.top, getWidth() - ins.left - ins.right - 1, getHeight() - ins.top - ins.bottom - 1);
			g.drawRect(
					ins.left + 1,
					ins.top + 1,
			...
			try {
				...
				g2.fill(new Ellipse2D.Float(
						inset + border, inset + border, DIAM - 2 * inset - 2 * border, DIAM - 2 * inset - 2 * border));
			} finally {
				g2.dispose();
			}
		}
