...
public class JXTable extends JTable implements TableColumnModelExtListener {
	...
	public void setColumnControlVisible(boolean visible) {
		if (isColumnControlVisible() == visible) return;
	...
	private void initFocusBindings() {
		...
		getInputMap(WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
				.put(KeyStroke.getKeyStroke("shift ctrl TAB"), FOCUS_PREVIOUS_COMPONENT);
			...
			getSortController()
					.setComparator(
							...
							tableColumn instanceof TableColumnExt
									? ((TableColumnExt) tableColumn).getComparator()
									: null);
		}
	}
	...
	public SortOrder getSortOrder(Object identifier) {
		if (!hasSortController()) return SortOrder.UNSORTED;
		TableColumn columnExt = getColumnByIdentifier(identifier);
	...
	public void columnAdded(TableColumnModelEvent e) {
		...
		updateSortableAfterColumnChanged(
				column, column instanceof TableColumnExt ? ((TableColumnExt) column).isSortable() : true);
	...
	public void columnPropertyChange(PropertyChangeEvent event) {
		if (event.getPropertyName().equals("editable")) {
			updateEditingAfterColumnChanged((TableColumn) event.getSource(), (Boolean) event.getNewValue());
		} else if (event.getPropertyName().equals("sortable")) {
			updateSortableAfterColumnChanged((TableColumn) event.getSource(), (Boolean) event.getNewValue());
	...
	public void packTable(int margin) {
		for (int c = 0; c < getColumnCount(); c++) packColumn(c, margin, -1);
	}
	...
	public void setDefaultRenderer(Class<?> columnClass, TableCellRenderer renderer) {
		...
		getStringValueRegistry()
				.setStringValue((renderer instanceof StringValue) ? (StringValue) renderer : null, columnClass);
	}
	...
	public String getStringAt(int row, int column) {
		...
		StringValue stringValue =
				getStringValueRegistry().getStringValue(convertRowIndexToModel(row), convertColumnIndexToModel(column));
	...
	protected void resetDefaultTableCellRendererColors(Component renderer, int row, int column) {
		if (!Boolean.TRUE.equals(getClientProperty(USE_DTCR_COLORMEMORY_HACK))) return;
		ComponentAdapter adapter = getComponentAdapter(row, column);
	...
	protected void createDefaultRenderers() {
		...
		TableCellRenderer renderer =
				new DefaultTableRenderer(new MappedValue(StringValues.EMPTY, IconValues.ICON), JLabel.CENTER);
	...
	public static class BooleanEditor extends DefaultCellEditor {
		...
		public Component getTableCellEditorComponent(
			...
			Component comp =
					super.getTableCellEditorComponent(table, value, isSelected || shouldSelectCell(null), row, column);
			return comp;
		}
	}
	...
	public void transferFocus() {
		if (isEditingFocusCycleRoot() && !getCellEditor().stopCellEditing()) return;
		super.transferFocus();
	}
	...
	public void updateUI() {
		...
		for (Enumeration<?> defaultEditors = defaultEditorsByColumnClass.elements();
				defaultEditors.hasMoreElements(); ) {
			updateEditorUI(defaultEditors.nextElement());
		}

		for (Enumeration<?> defaultRenderers = defaultRenderersByColumnClass.elements();
				defaultRenderers.hasMoreElements(); ) {
	...
	private void updateEditorUI(Object maybeEditor) {
		...
		if (!(maybeEditor instanceof TableCellEditor)) return;
		// super handled this
		if ((maybeEditor instanceof JComponent) || (maybeEditor instanceof DefaultCellEditor)) return;
	...
	private void updateRendererUI(Object maybeRenderer) {
		// maybe null or proxyValue
		if (!(maybeRenderer instanceof TableCellRenderer)) return;
		// super handled this
		...
		if (maybeRenderer instanceof AbstractRenderer abstractRenderer) {
			...
			try {
				...
				comp = ((TableCellRenderer) maybeRenderer)
						.getTableCellRendererComponent(this, null, false, false, -1, -1);
