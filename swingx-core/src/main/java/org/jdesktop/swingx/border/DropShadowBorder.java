...
public class DropShadowBorder implements Border, Serializable {
	...
	public void paintBorder(Component c, Graphics graphics, int x, int y, int width, int height) {
		...
		try {
			...
			if (showRightShadow || showBottomShadow) {
				g2.drawImage(
						images.get(Position.BOTTOM_RIGHT), bottomRightShadowPoint.x, bottomRightShadowPoint.y, null);
			}
			if (showRightShadow || showTopShadow) {
	...
	private Map<Position, BufferedImage> getImages(Graphics2D g2) {
		...
		Map<Position, BufferedImage> images =
				CACHE.get(shadowSize + (shadowColor.hashCode() * .3) + (shadowOpacity * .12)); // TODO do a real hash
		if (images == null) {
			images = new HashMap<>();
			...
			try {
				buffer.setPaint(new Color(shadowColor.getRed(), shadowColor.getGreen(), shadowColor.getBlue(), (int)
						(shadowOpacity * 255)));
			...
			image.flush();
			CACHE.put(
					shadowSize + (shadowColor.hashCode() * .3) + (shadowOpacity * .12), images); // TODO do a real hash
		}
		return images;
	}
