...
public class BasicMonthViewUI extends MonthViewUI {
	...
	private void installKeyBindings(int type) {
		...
		inputMap.put(
				KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, InputEvent.SHIFT_MASK, false), "adjustSelectionPreviousDay");
		inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, InputEvent.SHIFT_MASK, false), "adjustSelectionNextDay");
		inputMap.put(
				KeyStroke.getKeyStroke(KeyEvent.VK_UP, InputEvent.SHIFT_MASK, false), "adjustSelectionPreviousWeek");
	...
	protected Rectangle getDayBoundsAtLocation(int x, int y) {
		...
		if ((monthDetails == null) || (!monthDetails.contains(x, y))) return null;
		...
		return new Rectangle(
				monthDetails.x + column * fullBoxWidth,
				monthDetails.y + row * fullBoxHeight,
				fullBoxWidth,
				fullBoxHeight);
	}
	...
	public Date getDayAtLocation(int x, int y) {
		...
		if ((dayInGrid == null) || (dayInGrid.x == WEEK_HEADER_COLUMN) || (dayInGrid.y == DAY_HEADER_ROW)) return null;
	...
	protected int getTraversableGridPositionAtLocation(int x, int y) {
		...
		headerBounds.setBounds(
				headerBounds.x + arrowPaddingX, y, headerBounds.width - 2 * arrowPaddingX, headerBounds.height);
		if (!headerBounds.contains(x, y)) return -1;
		Rectangle hitArea = new Rectangle(
				headerBounds.x, headerBounds.y, monthUpImage.getIconWidth(), monthUpImage.getIconHeight());
	...
	private class Handler
		...
		public void layoutContainer(Container parent) {
			...
			for (int i = calendar.getMinimum(Calendar.MONTH); i <= calendar.getMaximum(Calendar.MONTH); i++) {
				...
				CalendarUtils.startOfMonth(calendar);
				JComponent comp =
						getRenderingHandler().prepareRenderingComponent(monthView, calendar, CalendarState.TITLE);
			...
			for (int i = 0; i < JXMonthView.DAYS_IN_WEEK; i++) {
				JComponent comp =
						getRenderingHandler().prepareRenderingComponent(monthView, calendar, CalendarState.DAY_OF_WEEK);
				Dimension pref = comp.getPreferredSize();
			...
			for (int i = 0; i < calendar.getMaximum(Calendar.DAY_OF_MONTH); i++) {
				JComponent comp =
						getRenderingHandler().prepareRenderingComponent(monthView, calendar, CalendarState.IN_MONTH);
				Dimension pref = comp.getPreferredSize();
				maxBoxWidth = Math.max(maxBoxWidth, pref.width);
				maxBoxHeight = Math.max(maxBoxHeight, pref.height);
				calendar.add(Calendar.DATE, 1);
