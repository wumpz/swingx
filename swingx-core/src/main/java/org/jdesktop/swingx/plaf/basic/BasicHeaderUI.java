...
public class BasicHeaderUI extends HeaderUI {
	...
	protected void installDefaults(JXHeader header) {
		...
		if (isUIInstallable(header.getDescriptionForeground())) {
			...
			// fallback to label foreground
			header.setDescriptionForeground(
					descForeground != null ? descForeground : UIManagerExt.getColor("Label.foreground"));
		}
		...
		boundsListener = new HierarchyBoundsAdapter() {
			...
			public void ancestorResized(HierarchyEvent e) {
				if (header == e.getComponent()) {
					...
					if (v == null) {
						descriptionPane.putClientProperty(
								BasicHTML.propertyKey, MultiLineSupport.createView(descriptionPane));
					...
					if (v != null) {
						...
						w -= 35
								+ header.getInsets().left
								+ header.getInsets().right
								+ descriptionPane.getInsets().left
								+ descriptionPane.getInsets().right
								+ imagePanel.getInsets().left
								+ imagePanel.getInsets().right
								+ imagePanel.getWidth()
								+ descriptionPane.getBounds().x;
						v.setSize(w, h);
