...
public class WindowsStatusBarUI extends BasicStatusBarUI {
	...
	public WindowsStatusBarUI() {
		...
		try {
			leftImage =
					ImageIO.read(WindowsStatusBarUI.class.getResource(UIManagerExt.getString("StatusBar.leftImage")));
			middleImage =
					ImageIO.read(WindowsStatusBarUI.class.getResource(UIManagerExt.getString("StatusBar.middleImage")));
			rightImage =
					ImageIO.read(WindowsStatusBarUI.class.getResource(UIManagerExt.getString("StatusBar.rightImage")));
		} catch (Exception e) {
	...
	protected void paintBackground(Graphics2D g, JXStatusBar statusBar) {
		...
		if (statusBar.isResizeHandleEnabled()) {
			g.drawImage(
					...
					leftImage.getWidth(),
					0,
					statusBar.getWidth() - leftImage.getWidth() - rightImage.getWidth(),
					statusBar.getHeight(),
					null);
			g.drawImage(
					rightImage,
					statusBar.getWidth() - rightImage.getWidth(),
					0,
					rightImage.getWidth(),
					statusBar.getHeight(),
					null);
			...
			g.drawImage(
					...
					leftImage.getWidth(),
					0,
					statusBar.getWidth() - leftImage.getWidth(),
					statusBar.getHeight(),
					null);
		}
	}
