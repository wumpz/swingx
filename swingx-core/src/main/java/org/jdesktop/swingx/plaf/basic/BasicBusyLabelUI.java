	...
	public BusyPainter getBusyPainter(final Dimension dim) {
		BusyPainter p = new BusyPainter() {
			...
			protected void init(Shape point, Shape trajectory, Color b, Color h) {
				super.init(
						dim == null
								? UIManagerExt.getShape("JXBusyLabel.pointShape")
								: getScaledDefaultPoint(dim.height),
						dim == null
								? UIManagerExt.getShape("JXBusyLabel.trajectoryShape")
								: getScaledDefaultTrajectory(dim.height),
