...
public class BasicTaskPaneUI extends TaskPaneUI {
	...
	protected class PaneBorder implements Border, UIResource {
			...
			if (isMouseOverBorder()) {
				if (mouseOver) {
					...
					if (group.isSpecial()) {
						...
						paintColor = group.getForeground() == null || group.getForeground() instanceof ColorUIResource
								? titleForeground
								: group.getForeground();
					}
				}
		 ...
		 *            Height of the box.
		 */
		protected void paintRectAroundControls(
				JXTaskPane group, Graphics g, int x, int y, int width, int height, Color highColor, Color lowColor) {
			if (mouseOver) {
				int x2 = x + width;
				int y2 = y + height;
