...
public final class ListSortUI {
	...
	public ListSortUI(JXList list, RowSorter<? extends ListModel> sorter) {
		...
		if (sorter != list.getRowSorter()) throw new IllegalStateException("sorter must be same as the one on list");
	...
	private void cacheModelSelection(RowSorterEvent sortEvent) {
		...
		modelLeadIndex =
				convertRowIndexToModel(sortEvent, getViewSelectionModel().getLeadSelectionIndex());
	}
