...
public class SearchFieldUI extends BuddyTextFieldUI {
	...
	protected void updateButtons() {
		...
		searchButton()
				.setVisible((searchField.getFindPopupMenu() == null || usingSeperatePopupButton()) && clearNotHere);
		popupButton()
				.setVisible(searchField.getFindPopupMenu() != null && (clearNotHere || usingSeperatePopupButton()));
	...
	class Handler implements PropertyChangeListener, ActionListener, DocumentListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (searchField.getFindPopupMenu() != null) {
				Component src =
						SearchFieldAddon.SEARCH_FIELD_SOURCE.equals(UIManager.getString("SearchField.popupSource"))
								? searchField
								: (Component) e.getSource();

				Rectangle r = SwingUtilities.getLocalBounds(src);
				int popupWidth = searchField.getFindPopupMenu().getPreferredSize().width;
				int x = searchField.isVistaLayoutStyle() || usingSeperatePopupButton()
						? r.x + r.width - popupWidth
						: r.x;
