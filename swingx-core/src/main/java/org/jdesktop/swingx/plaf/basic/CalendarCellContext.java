...
class CalendarCellContext extends CellContext {
	...
	public void installContext(
			JXMonthView component, Calendar value, boolean selected, boolean focused, CalendarState dayState) {
	...
	protected Color getForeground() {
		...
		if (CalendarState.DAY_OF_WEEK == dayState) {
			Color daysOfTheWeekForeground =
					getComponent() != null ? getComponent().getDaysOfTheWeekForeground() : null;
	...
	protected Color getFlaggedOrPerDayForeground() {
		...
		if (getComponent() != null && (getCalendar() != null)) {
			if (getComponent().isFlaggedDate(getCalendar().getTime())) {
				...
				Color perDay =
						getComponent().getPerDayOfWeekForeground(getCalendar().get(Calendar.DAY_OF_WEEK));
				if (perDay != null) {
					return perDay;
				}
			}
	...
	protected Border getBorder() {
		...
		return BorderFactory.createEmptyBorder(
				getComponent().getBoxPaddingY(),
				getComponent().getBoxPaddingX(),
				getComponent().getBoxPaddingY(),
				getComponent().getBoxPaddingX());
	}
