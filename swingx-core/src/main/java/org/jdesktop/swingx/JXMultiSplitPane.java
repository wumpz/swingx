		...
		if (divider != null) {
			...
			if ((prevNode == null) || (nextNode == null)) {
				...
				if (dragDivider.isVertical()) {
					...
					if (msl.getLayoutMode() == MultiSplitLayout.USER_MIN_SIZE_LAYOUT) dragMax -= msl.getUserMinSize();
				} else {
				...
				if (msl.getLayoutMode() == MultiSplitLayout.USER_MIN_SIZE_LAYOUT) {
					...
					if (dragDivider.isVertical()) {
						...
						if (maxDim != null) dragMax = Math.min(dragMax, prevNodeBounds.x + maxDim.width);
					} else {
						...
						Dimension maxDim = getMaxNodeSize(msl, prevNode);
						if (maxDim != null) dragMax = Math.min(dragMax, prevNodeBounds.y + maxDim.height);
					}
				}
