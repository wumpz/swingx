...
public class JXLabel extends JLabel implements BackgroundPaintable {
	...
	// using reverse logic ... some methods causing re-flow of text are called from super constructor, but private
	// variables are initialized only after call to super so have to rely on default for boolean being false
	...
	private void initLineWrapSupport() {
		...
		addHierarchyBoundsListener(new HierarchyBoundsAdapter() {
			...
			public void ancestorResized(HierarchyEvent e) {
				// if one of the parents is viewport, resized events will not be propagated down unless viewport is
				// changing visibility of scrollbars.
				// To make sure Label is able to re-wrap text when viewport size changes, initiate re-wrapping here by
				// changing size of view
	...
	public Dimension getPreferredSize() {
		...
		if (isPreferredSizeSet()) {
			...
			// #swingx-680 change the preferred size when rotation is set ... ideally this would be solved in the
			// LabelUI rather then here
	...
	static class Renderer extends WrappedPlainView {
		...
		Renderer(JXLabel c, ViewFactory f, View v, boolean wordWrap) {
			...
			// setSize(c.getMaxLineSpan() > -1 ? c.getMaxLineSpan() : view.getPreferredSpan(X_AXIS),
