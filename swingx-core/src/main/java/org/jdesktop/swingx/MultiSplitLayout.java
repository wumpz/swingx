...
public class MultiSplitLayout implements LayoutManager, Serializable {
	...
	private void doLayoutByWeight(Container parent) {
		...
		if (model instanceof Leaf) model.setBounds(bounds);
		else if (model instanceof Split) doLayoutByWeight(model, bounds);
	}
	...
	private void doLayoutByWeight(Node node, Rectangle bounds) {
		...
		for (Node splitChild : splitChildren) {
			...
			if (weight > 0.0) distributableWeight -= weight;
			else unweightedComponents++;
		}
		...
		if (split.isRowLayout()) {
			...
			for (Node splitChild : splitChildren) {
				...
				if (weight >= 0)
					splitChildBounds =
							new Rectangle(splitChildBounds.x, splitChildBounds.y, (int) (width * weight), height);
				else
					splitChildBounds = new Rectangle(
							splitChildBounds.x,
							splitChildBounds.y,
							(int) (distributableWidth / unweightedComponents),
							height);
				...
				if (splitChild instanceof Split) doLayoutByWeight(splitChild, splitChildBounds);
				else {
					Component comp = getComponentForNode(splitChild);
					if (comp != null) comp.setPreferredSize(splitChildBounds.getSize());
				}
			}
		} else {
			height -= dividerSpace;
			...
			for (Node splitChild : splitChildren) {
				...
				if (weight >= 0)
					splitChildBounds =
							new Rectangle(splitChildBounds.x, splitChildBounds.y, width, (int) (height * weight));
				else
					splitChildBounds = new Rectangle(splitChildBounds.x, splitChildBounds.y, width, (int)
							(distributableHeight / unweightedComponents));
	...
	public void removeLayoutNode(String name) {
		...
		if (name != null) {
			Node n;
			if (!(model instanceof Split)) n = model;
			else n = getNodeForName(name);
	...
	public void displayNode(String name, boolean visible) {
		...
		if (node != null) {
			...
			if (!visible) {
				p.hide(node);
				if (!p.isVisible()) p.getParent().hide(p);
				...
				while (!p.isVisible()) {
					p = p.getParent();
					if (p != null) p.checkDividers(p);
					else break;
				}
			} else p.restoreDividers(p);
		}
	...
	private Dimension preferredComponentSize(Node node) {
		if (layoutMode == NO_MIN_SIZE_LAYOUT) return new Dimension(0, 0);
		...
		if (split.isRowLayout()) {
			...
			while (splitChildren.hasNext()) {
				...
				if (!splitChild.isVisible()) {
					if (splitChildren.hasNext()) splitChildren.next();
					continue;
				}
				Rectangle splitChildBounds = splitChild.getBounds();
	...
	public void layoutContainer(Container parent) {
		if (layoutByWeight && floatingDividers) doLayoutByWeight(parent);
	...
	public static class Split extends Node {
		...
		public boolean isVisible() {
			for (Node child : children) {
				if (child.isVisible() && !(child instanceof Divider)) return true;
			}
			return false;
		}
		...
		public void remove(Node n) {
			if (n.nextSibling() instanceof Divider) children.remove(n.nextSibling());
			else if (n.previousSibling() instanceof Divider) children.remove(n.previousSibling());
			children.remove(n);
		}
		...
		public void hide(Node target) {
			...
			if (next instanceof Divider) next.setVisible(false);
			else {
				Node prev = target.previousSibling();
				if (prev instanceof Divider) prev.setVisible(false);
			}
				...
				if (!splitChild.isVisible()) {
					...
					if (dividerChild instanceof Divider) {
						if (splitChildren.hasNext()) {
							...
							if ((rightChild != null) && (rightChild instanceof Divider)) dividerChild.setVisible(false);
						}
		...
		public void restoreDividers(Split split) {
			...
			if (split.getParent() != null) restoreDividers(split.getParent());
		}
