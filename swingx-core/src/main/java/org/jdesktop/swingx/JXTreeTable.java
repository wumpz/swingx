...
public class JXTreeTable extends JXTable {
	...
	public void setSelectionBackground(Color selectionBackground) {
		...
		if (renderer != null) renderer.setSelectionBackground(selectionBackground);
	...
	public void setSelectionForeground(Color selectionForeground) {
		// happens on instantiation, updateUI is called before the renderer is installed
		if (renderer != null) renderer.setSelectionForeground(selectionForeground);
	...
	public class TreeTableHackerExt4 extends TreeTableHackerExt {
		...
		public boolean hitHandleDetectionFromProcessMouse(MouseEvent e) {
			if (!isHitDetectionFromProcessMouse()) return false;
	...
	public class TreeTableHacker {
		...
		protected boolean expandOrCollapseNode(int column, EventObject e) {
			...
			if (!mightBeExpansionTrigger(e)) return false;
	...
	protected static class TreeTableModelAdapter extends AbstractTableModel implements TreeTableModelProvider {
		...
		private boolean isTableStructureChanged(TreeModelEvent e) {
			if ((e.getTreePath() == null) || (e.getTreePath().getParentPath() == null)) return true;
			return false;
		}
				...
				public void run() {
					...
					if (indices != null) {
						if (expanded) { // Dont bother to update if the parent
							...
							//                            LOG.info("Updated: parentPath/min/max" + path + "/" + min +
							// "/" + max);
	...
	//      ,  RolloverRenderer
	{
		...
		protected PropertyChangeListener createRolloverListener() {
			PropertyChangeListener l = new PropertyChangeListener() {
				...
				@Override
				public void propertyChange(PropertyChangeEvent evt) {
					if ((treeTable == null) || (treeTable != evt.getSource())) return;
				...
				private void rollover(PropertyChangeEvent evt) {
					...
					putClientProperty(
							evt.getPropertyName(), isHierarchical ? new Point((Point) evt.getNewValue()) : null);
				}
		...
		private class ClippedTreeCellRenderer extends DefaultXTreeCellRenderer implements StringValue {
			...
			public Component getTreeCellRendererComponent(
				...
				return super.getTreeCellRendererComponent(
						tree, getHierarchicalTableValue(value), sel, expanded, leaf, row, hasFocus);
			}
			...
			public String getString(Object node) {
				...
				//                if (treeColumn >= 0) {
				//                    return
				// StringValues.TO_STRING.getString(treeTable.getTreeTableModel().getValueAt(value, treeColumn));
				//                }
