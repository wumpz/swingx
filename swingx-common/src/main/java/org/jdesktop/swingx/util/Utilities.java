...
public final class Utilities {
	...
	public static final int OS_UNIX_MASK =
			OS_SOLARIS | OS_LINUX | OS_HP | OS_AIX | OS_IRIX | OS_SUNOS | OS_TRU64 | OS_MAC | OS_FREEBSD;
	...
	public static Object toPrimitiveArray(Object[] array) {
		if (array instanceof Integer[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] == null) ? 0 : ((Integer) array[i]);
		...
		if (array instanceof Boolean[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] != null) && ((Boolean) array[i]);
		...
		if (array instanceof Byte[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] == null) ? 0 : ((Byte) array[i]);
		...
		if (array instanceof Character[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] == null) ? 0 : ((Character) array[i]);
		...
		if (array instanceof Double[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] == null) ? 0 : ((Double) array[i]);
		...
		if (array instanceof Float[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] == null) ? 0 : ((Float) array[i]);
		...
		if (array instanceof Long[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] == null) ? 0 : ((Long) array[i]);
		...
		if (array instanceof Short[]) {
			...
			for (i = 0; i < k; i++) r[i] = (array[i] == null) ? 0 : ((Short) array[i]);
	...
	public static Object[] toObjectArray(Object array) {
		...
		if (array instanceof byte[]) {
			...
			for (i = 0; i < k; i++) r[i] = ((byte[]) array)[i];
		...
		if (array instanceof char[]) {
			...
			for (i = 0; i < k; i++) r[i] = ((char[]) array)[i];
		...
		if (array instanceof double[]) {
			...
			for (i = 0; i < k; i++) r[i] = ((double[]) array)[i];
		...
		if (array instanceof float[]) {
			...
			for (i = 0; i < k; i++) r[i] = ((float[]) array)[i];
		...
		if (array instanceof long[]) {
			...
			for (i = 0; i < k; i++) r[i] = ((long[]) array)[i];
		...
		if (array instanceof short[]) {
			...
			for (i = 0; i < k; i++) r[i] = ((short[]) array)[i];
	...
	public static String[] wrapStringToArray(
		...
		widthcheck:
		{
			...
			for (String workingSet1 : workingSet) {
				ok = ok && (workingSet1.length() < width);
				if (!ok) {
					break widthcheck;
				}
			}
		...
		for (String workingSet1 : workingSet) {
			if (workingSet1.length() < width) {
				lines.add(workingSet1);
			} else {
				breakIterator.setText(workingSet1);
				...
				do {
					...
					if (nextStart == BreakIterator.DONE) {
						nextStart = prevStart = workingSet1.length();
					}
					if (prevStart == 0) {
						prevStart = nextStart;
					}
					lines.add(workingSet1.substring(lineStart, prevStart));
					lineStart = prevStart;
					prevStart = 0;
				} while (lineStart < workingSet1.length());
				lineStart = 0;
			}
		}
