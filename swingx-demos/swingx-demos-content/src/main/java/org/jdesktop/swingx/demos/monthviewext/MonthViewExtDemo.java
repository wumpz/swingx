...
public class MonthViewExtDemo extends JPanel {
	...
	private ComboBoxModel createHeaderInfos() {
		...
		model.addElement(new DisplayInfo<CalendarHeaderHandler>("default", new SpinningCalendarHeaderHandler()));
		model.addElement(new DisplayInfo<CalendarHeaderHandler>(
				"default (customized)", new DemoCalendarHeaderHandler(true, true)));
		// </snip>
		return model;
	}
	...
	public class MonthViewDemoControl extends AbstractBean {
		...
		public MonthViewDemoControl() {
			...
			group.addBinding(Bindings.createAutoBinding(
					...
					this,
					BeanProperty.create("calendarVisible")));

			group.addBinding(Bindings.createAutoBinding(
					READ, zoomableBox, BeanProperty.create("selected"), monthView, BeanProperty.create("zoomable")));

			// <snip> Custom CalendarHeaderHandler
			// bind the combo box
			group.addBinding(Bindings.createAutoBinding(
					READ, monthView, BeanProperty.create("zoomable"), customHeaderBox, BeanProperty.create("enabled")));
			Binding handlerBinding = Bindings.createAutoBinding(
					READ,
					customHeaderBox,
					BeanProperty.create("selectedItem"),
					this,
					BeanProperty.create("calendarHeaderHandler"));
	...
	private JComponent createExtendedConfigPanel() {
		...
		JLabel headerBoxLabel = builder.addLabel(
				"", cl.xywh(labelColumn, currentRow, 1, 1), customHeaderBox, cc.xywh(widgetColumn, currentRow, 1, 1));
