...
public class DecoratorDemo extends JPanel {
	...
	private void updateDecorators() {
		...
		RowFilter listFilters = findModi[1].equals(getFindModus()) ? getFilterPipeline("list") : null;
		list.setRowFilter(listFilters);

		// filter/search support is implemented for JXTable only
		RowFilter filters = findModi[1].equals(getFindModus()) ? getFilterPipeline("table") : null;
	...
	private void createHighlighters() {
		...
		rolloverHighlighter = new ColorHighlighter(HighlightPredicate.ROLLOVER_ROW, Color.LIGHT_GRAY, null);
		//        </snip>
	...
	public void setFindModus(String value) {
		if (value.equals(getFindModus())) return;
	...
	public void findNext() {
		int foundIndex = table.getSearchable().search(patternModel.getPattern(), patternModel.getFoundIndex());
	...
	private void build() {
		...
		builder.addLabel("JXList:", cl.xywh(3, 1, 1, 1), new JScrollPane(list), cc.xywh(3, 3, 1, 1));
		builder.addLabel("JXTree:", cl.xywh(5, 1, 1, 1), new JScrollPane(tree), cc.xywh(5, 3, 1, 1));
	...
	private JComponent buildControl() {
		...
		CellConstraints cl = new CellConstraints();
		CellConstraints cc = new CellConstraints();
		radioGroupLabel = builder.addLabel("", cl.xywh(1, 1, 1, 1), radioGroup, cc.xywh(3, 1, 3, 1));
		inputTextLabel = builder.addLabel("", cl.xywh(1, 3, 1, 1), inputText, cc.xywh(3, 3, 1, 1));
		patternLabel = builder.addLabel("", cl.xywh(1, 5, 1, 1), patternText, cc.xywh(3, 5, 1, 1));
