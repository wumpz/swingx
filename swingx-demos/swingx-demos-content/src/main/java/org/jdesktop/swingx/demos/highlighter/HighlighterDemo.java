...
public class HighlighterDemo extends DefaultDemoPanel {
	...
	private ComboBoxModel getStripingOptionsModel() {
		...
		info.add(new HighlighterInfo(
				"Alternating Groups (2) UI-Dependent", HighlighterFactory.createAlternateStriping(2)));
		info.add(new HighlighterInfo(
				"Alternating Groups (3) UI-Dependent", HighlighterFactory.createAlternateStriping(3)));

		Color lightBlue = new Color(0xC0D9D9);
		Color gold = new Color(0xDBDB70);
		info.add(new HighlighterInfo(
				"Alternating Blue-Gold", HighlighterFactory.createAlternateStriping(lightBlue, gold)));
		info.add(new HighlighterInfo(
				"Alternating Groups (2) Blue-Gold", HighlighterFactory.createAlternateStriping(lightBlue, gold, 2)));
		info.add(new HighlighterInfo(
	...
	private ComboBoxModel getHighlighterOptionsModel() {
		...
		info.add(new HighlighterInfo(
				"Blue Border", new BorderHighlighter(BorderFactory.createLineBorder(Color.BLUE, 1))));
		...
		info.add(new HighlighterInfo(
				"Green Orb Icon",
				new IconHighlighter(Application.getInstance()
						.getContext()
						.getResourceMap(HighlighterDemo.class)
						.getIcon("greenOrb"))));
		info.add(new HighlighterInfo(
				"Aerith Gradient Painter", new PainterHighlighter(new MattePainter(PaintUtils.AERITH, true))));
		info.add(new HighlighterInfo(
				"Star Shape Painter",
				new PainterHighlighter(
						new ShapePainter(ShapeUtils.generatePolygon(5, 10, 5, true), PaintUtils.NIGHT_GRAY_LIGHT))));
	...
	private ComboBoxModel getPredicateOptionsModel() {
		...
		info.add(new HighlightPredicateInfo(
				"Node Depth Columns 0 and 3", new HighlightPredicate.DepthHighlightPredicate(0, 3)));
		info.add(new HighlightPredicateInfo(
	...
	protected void bind() {
		...
		Binding b = Bindings.createAutoBinding(
				READ,
				predicates,
				ELProperty.create("${selectedItem.predicate}"),
				highlighters,
		...
		activeHighlighters.addSource(stripingOptions, (Property) ELProperty.create("${selectedItem.highlighter}"));
		activeHighlighters.addSource(highlighters, (Property) ELProperty.create("${selectedItem.highlighter}"));
		...
		Bindings.createAutoBinding(
						...
						list,
						BeanProperty.create("highlighters"))
				.bind();
