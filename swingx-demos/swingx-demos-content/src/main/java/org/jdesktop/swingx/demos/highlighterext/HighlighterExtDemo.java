...
public class HighlighterExtDemo extends JPanel {
	...
	public class HighlighterControl extends AbstractBean {
		...
		public HighlighterControl() {
			...
			group.addBinding(Bindings.createAutoBinding(
					...
					this,
					BeanProperty.create("spreadColumns")));
			group.bind();
		}
		...
		public void fadeIn() {
			if (fadeInTimeline == null) {
				...
				fadeInTimeline.addPropertyToInterpolate(
						"background", PaintUtils.setAlpha(base, 0), PaintUtils.setAlpha(base, 125));
	...
	private void installRenderers() {
		...
		HyperlinkProvider provider = new HyperlinkProvider(new ContributorMailAction(stringValues.get("email")));
		installColumnRenderers(keys.length - 1, new DefaultTableRenderer(provider));
		table.getColumnExt(keys.length - 1)
				.setToolTipText("Note: the mail-to action will do nothing in security restricted environments");
		// </snip>
		table.packAll();
	}
	...
	public static class ContributorMailAction extends AbstractHyperlinkAction<Contributor> {
		HyperlinkAction browse = HyperlinkAction.createHyperlinkAction(null, java.awt.Desktop.Action.MAIL);
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				JFrame frame = new JFrame(HighlighterExtDemo.class
