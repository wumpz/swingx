...
public class XMatchingTextHighlighter extends MatchingTextHighlighter {
		...
		if (!fullText.equals(clippedText)) {
			...
			if (matchesInFullText > matchesInClippedText) { // happens if at least one match is under the ellipsis
				...
				areas.add(new Rectangle(
						beginOfEllipsis, yOffset, fm.stringWidth("..."), height)); // hopefully no one ever changes
	...
	protected Component doHighlight(Component component, ComponentAdapter adapter) {
		if (isLabelCompatible(component)) return super.doHighlight(component, adapter);
	...
	protected boolean canHighlight(Component component, ComponentAdapter adapter) {
		...
		// &&
		component instanceof PainterAware && getPainter() != null && getHighlightPredicate() instanceof SearchPredicate;
	}

	protected boolean isLabelCompatible(Component component) {
		return component instanceof JLabel
				|| (component instanceof WrappingIconPanel
						&& ((WrappingIconPanel) component).getComponent() instanceof JLabel);
	}
}
