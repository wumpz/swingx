...
public class XListDemo extends JXPanel {
	...
	public void setRolloverEnabled(boolean enabled) {
		...
		list.setToolTipText(
				list.isRolloverEnabled() ? DemoUtils.getResourceString(getClass(), "stickyRolloverToolTip") : null);
	}
	// ------------------- ui configuration
	...
	private void bind() {
		...
		group.addBinding(Bindings.createAutoBinding(
				...
				this,
				BeanProperty.create("rolloverEnabled")));
		Binding comparatorBinding = Bindings.createAutoBinding(
				READ, comparatorCombo, BeanProperty.create("selectedItem"), this, BeanProperty.create("comparator"));
		comparatorBinding.setConverter(new DisplayInfoConverter<Comparator<?>>());
		group.addBinding(comparatorBinding);

		Binding rolloverBinding = Bindings.createAutoBinding(
				READ,
				highlighterCombo,
				BeanProperty.create("selectedItem"),
				this,
				BeanProperty.create("rolloverHighlighter"));
	...
	private ComboBoxModel createRolloverHighlighters() {
		...
		model.addElement(new DisplayInfo<Highlighter>(
				"Background Color", new ColorHighlighter(HighlightPredicate.ROLLOVER_ROW, Color.MAGENTA, null)));
		model.addElement(new DisplayInfo<Highlighter>(
	...
	private ComboBoxModel createComparators() {
		...
		model.addElement(new DisplayInfo<Comparator<?>>(
				"Comparable (by lastname)", DefaultSortController.COMPARABLE_COMPARATOR));
		// custom comparator
	...
	private JComponent createExtendedConfigPanel() {
		...
		JLabel comparatorComboLabel = builder.addLabel(
				"", cl.xywh(labelColumn, currentRow, 1, 1), comparatorCombo, cc.xywh(widgetColumn, currentRow, 1, 1));
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				JFrame frame = new JFrame(
						XListDemo.class.getAnnotation(DemoProperties.class).value());
