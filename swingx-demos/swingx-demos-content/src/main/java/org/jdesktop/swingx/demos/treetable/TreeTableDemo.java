...
public class TreeTableDemo extends JPanel {
		...
		ColumnFactory factory = new ColumnFactory() {
			...
			public void configureTableColumn(TableModel model, TableColumnExt columnExt) {
				...
				if (columnExt.getModelIndex() < columnNameKeys.length) {
					columnExt.setTitle(DemoUtils.getResourceString(
							TreeTableDemo.class, columnNameKeys[columnExt.getModelIndex()]));
				}
			}
		};
	...
	private void configureComponents() {
		...
		mouseOverHighlighter =
				new ColorHighlighter(HighlightPredicate.NEVER, PaintUtils.setSaturation(Color.MAGENTA, 0.3f), null);
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				JFrame frame = new JFrame(
						TreeTableDemo.class.getAnnotation(DemoProperties.class).value());
