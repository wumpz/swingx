...
public class DemoCreator {
	...
	private List<String> readDemoClassNames(Reader reader) throws IOException {
		...
		while ((line = breader.readLine()) != null) {
			if (isClassName(line)) demoClassNames.add(line);
		}
		breader.close();
	...
	private boolean isClassName(String line) {
		if ((line == null) || line.startsWith("#") || line.trim().length() == 0) return false;
		return true;
	}
	...
	private List<String> getDemoClassNames(String args[], String metaSource) {
		...
		if (demoList.isEmpty() || augment) {
			...
			try {
				demoList.addAll(
						readDemoClassNames(new InputStreamReader(getClass().getResourceAsStream(metaSource))));
			} catch (IOException ex) {
				exception = ex;
